...
select tmp.user_id, p.id as party_id, null as branch_id, 1 as type, uv.username, uv.realname, uv.code from (
      select * from (
  ...
  union all
  select tmp.user_id, p.id as party_id, b.id as branch_id, 2 as type, uv.username, uv.realname, uv.code from (
      select * from (
