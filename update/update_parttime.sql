...
INSERT INTO `sys_resource` (`id`, `is_mobile`, `name`, `remark`, `type`, `menu_css`, `url`, `parent_id`, `parent_ids`, `is_leaf`, `permission`, `role_count`, `count_cache_keys`, `count_cache_roles`, `available`, `sort_order`) VALUES (5009, 0, '审批权限管理', '', 'url', '', '/parttimeApproveAuth', 5007, '0/1/862/5007/', 1, 'parttimeApproveAuth:*', 4, NULL, NULL, 1, NULL);
...
INSERT INTO `sys_resource` (`id`, `is_mobile`, `name`, `remark`, `type`, `menu_css`, `url`, `parent_id`, `parent_ids`, `is_leaf`, `permission`, `role_count`, `count_cache_keys`, `count_cache_roles`, `available`, `sort_order`) VALUES (5007, 0, '兼职申报', '', 'menu', '', NULL, 862, '0/1/862/', 0, 'parttimeApply:menu', 4, NULL, NULL, 1, NULL);
...
CREATE TABLE `parttime_applicat_cadre` (
	...
	INDEX `FK_cadre_company_applicat_cadre_cadre_company_applicat_type` (`type_id`) USING BTREE,
	INDEX `FK_cadre_company_applicat_cadre_cadre` (`cadre_id`) USING BTREE,
	CONSTRAINT `FK_cadre_company_applicat_cadre_cadre` FOREIGN KEY (`cadre_id`) REFERENCES `db_owip`.`cadre` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `FK_cadre_company_applicat_cadre_cadre_company_applicat_type` FOREIGN KEY (`type_id`) REFERENCES `db_owip`.`parttime_applicat_type` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT
)
COMMENT='申请人分组'
COLLATE='utf8_general_ci'
ENGINE=InnoDB
...
CREATE TABLE `parttime_apply` (
	...
	PRIMARY KEY (`id`) USING BTREE,
	INDEX `FK_cadre_company_apply_cadre` (`cadre_id`) USING BTREE,
	CONSTRAINT `FK_cadre_company_apply_cadre` FOREIGN KEY (`cadre_id`) REFERENCES `db_owip`.`cadre` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT
)
COMMENT='兼职申报申请'
COLLATE='utf8_general_ci'
ENGINE=InnoDB
...
CREATE TABLE `parttime_approval_log` (
	...
	INDEX `user_id` (`user_id`) USING BTREE,
	INDEX `type_id` (`type_id`) USING BTREE,
	CONSTRAINT `FK_cadre_company_approval_log_cadre_company_apply` FOREIGN KEY (`apply_id`) REFERENCES `db_owip`.`parttime_apply` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `FK_cadre_company_approval_log_cadre_company_approver_type` FOREIGN KEY (`type_id`) REFERENCES `db_owip`.`parttime_approver_type` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `FK_cadre_company_approval_log_sys_user` FOREIGN KEY (`user_id`) REFERENCES `db_owip`.`sys_user` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT
)
COMMENT='兼职申报申请记录'
COLLATE='utf8_general_ci'
ENGINE=InnoDB
...
CREATE TABLE `parttime_approval_order` (
	...
	INDEX `applicate_type_id` (`applicate_type_id`) USING BTREE,
	INDEX `approver_type_id` (`approver_type_id`) USING BTREE,
	CONSTRAINT `FK_cadre_company_approval_order_cadre_company_applicat_type` FOREIGN KEY (`applicate_type_id`) REFERENCES `db_owip`.`parttime_applicat_type` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `FK_cadre_company_approval_order_cadre_company_approver_type` FOREIGN KEY (`approver_type_id`) REFERENCES `db_owip`.`parttime_approver_type` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE
)
COMMENT='申请人身份关联的审批顺序，除了组织部终审'
COLLATE='utf8_general_ci'
ENGINE=InnoDB
...
CREATE TABLE `parttime_approver` (
	...
	INDEX `cadre_id` (`cadre_id`) USING BTREE,
	INDEX `type_id` (`type_id`) USING BTREE,
	CONSTRAINT `FK__cadre_1` FOREIGN KEY (`cadre_id`) REFERENCES `db_owip`.`cadre` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `FK__cadre_company_approver_type` FOREIGN KEY (`type_id`) REFERENCES `db_owip`.`parttime_approver_type` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT
)
COMMENT='审批人'
COLLATE='utf8_general_ci'
ENGINE=InnoDB
...
CREATE TABLE `parttime_approver_black_list` (
	...
	INDEX `FK__cadre_company_approver_type1` (`approver_type_id`) USING BTREE,
	INDEX `cadre_id` (`cadre_id`) USING BTREE,
	CONSTRAINT `FK__cadre_company_approver_type1` FOREIGN KEY (`approver_type_id`) REFERENCES `db_owip`.`parttime_approver_type` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE,
	CONSTRAINT `FK_parttime_approver_black_list_cadre` FOREIGN KEY (`cadre_id`) REFERENCES `db_owip`.`cadre` (`id`) ON UPDATE RESTRICT ON DELETE CASCADE
)
COMMENT='审批人黑名单，根据审批人身份设定黑名单'
COLLATE='utf8_general_ci'
ENGINE=InnoDB
...
INSERT INTO `base_content_tpl` (`id`, `name`, `role_id`, `type`, `code`, `wx_msg_type`, `wx_title`, `wx_url`, `wx_pic`, `content`, `content_type`, `engine`, `param_count`, `param_names`, `param_def_values`, `sort_order`, `user_id`, `create_time`, `update_time`, `is_deleted`, `remark`) VALUES (73, '[兼职申报]初审', NULL, 1, 'ct_parttime_apply_submit_info', NULL, NULL, NULL, NULL, '{0}：您好！{1}{2}老师已提交兼职申报申请，请登录 http://zzgz.bnu.edu.cn 进行初审。谢谢！', 1, 1, NULL, NULL, NULL, 50, 100719, '2018-06-07 11:57:08', '2019-06-08 16:49:23', 0, '兼职申报审批');
INSERT INTO `base_content_tpl` (`id`, `name`, `role_id`, `type`, `code`, `wx_msg_type`, `wx_title`, `wx_url`, `wx_pic`, `content`, `content_type`, `engine`, `param_count`, `param_names`, `param_def_values`, `sort_order`, `user_id`, `create_time`, `update_time`, `is_deleted`, `remark`) VALUES (74, '[兼职申报]终审', NULL, 1, 'ct_parttime_apply_pass_info', NULL, NULL, NULL, NULL, '{0}：您好！{1}{2}老师兼职申报申请领导已完成审批，请登录 http://zzgz.bnu.edu.cn 进行终审。谢谢！', 1, 1, NULL, NULL, NULL, 51, 100719, '2018-06-07 11:57:41', '2019-06-08 16:49:18', 0, '兼职申报审批');
INSERT INTO `base_content_tpl` (`id`, `name`, `role_id`, `type`, `code`, `wx_msg_type`, `wx_title`, `wx_url`, `wx_pic`, `content`, `content_type`, `engine`, `param_count`, `param_names`, `param_def_values`, `sort_order`, `user_id`, `create_time`, `update_time`, `is_deleted`, `remark`) VALUES (75, '兼职申报报送提醒', NULL, 1, 'part_branch_parttime_master', NULL, NULL, NULL, NULL, '{0}：您好！{1}{2}老师的兼职申报申请，请您直接点击 http://zzgz.bnu.edu.cn 在手机浏览器上登陆系统进行审批，账号为工作证号，密码为信息门户登陆密码。也可以在电脑的浏览器上输入上述网址进行审批。联系电话：58808302、58806879。谢谢！[系统短信，请勿回复]', 1, NULL, NULL, NULL, NULL, 69, 100719, '2020-12-10 17:42:57', '2021-04-03 17:45:14', 0, '');
