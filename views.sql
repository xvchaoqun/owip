...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ext_member_view` AS select `u`.`code` AS `sid`,`ui`.`realname` AS `realname`,`om`.`status` AS `status`,`om`.`type` AS `type`,`om`.`political_status` AS `political_status` from ((`sys_user` `u` join `sys_user_info` `ui`) join `ow_member` `om`) where ((`om`.`user_id` = `u`.`id`) and (`ui`.`user_id` = `u`.`id`) and (not(`om`.`user_id` in (select `ow_graduate_abroad`.`user_id` from `ow_graduate_abroad` where (`ow_graduate_abroad`.`status` = 3))))) union all select `su`.`code` AS `sid`,`sui`.`realname` AS `realname`,(`oga`.`status` + 1) AS `status`,`om`.`type` AS `type`,`om`.`political_status` AS `political_status` from (((`ow_graduate_abroad` `oga` join `ow_member` `om`) join `sys_user` `su`) join `sys_user_info` `sui`) where ((`oga`.`status` = 3) and (`oga`.`user_id` = `om`.`user_id`) and (`oga`.`user_id` = `su`.`id`) and (`sui`.`user_id` = `su`.`id`));
...
-- ----------------------------
--  View definition for `ow_graduate_abroad_view`
-- ----------------------------
DROP VIEW IF EXISTS `ow_graduate_abroad_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ow_graduate_abroad_view` AS select `oga`.`id` AS `id`,`oga`.`user_id` AS `user_id`,`oga`.`party_id` AS `party_id`,`oga`.`branch_id` AS `branch_id`,`oga`.`to_branch_id` AS `to_branch_id`,`oga`.`org_branch_admin_id` AS `org_branch_admin_id`,`oga`.`org_branch_admin_phone` AS `org_branch_admin_phone`,`oga`.`user_type` AS `user_type`,`oga`.`abroad_reason` AS `abroad_reason`,`oga`.`mobile` AS `mobile`,`oga`.`phone` AS `phone`,`oga`.`weixin` AS `weixin`,`oga`.`email` AS `email`,`oga`.`qq` AS `qq`,`oga`.`in_address` AS `in_address`,`oga`.`out_address` AS `out_address`,`oga`.`name1` AS `name1`,`oga`.`relate1` AS `relate1`,`oga`.`unit1` AS `unit1`,`oga`.`post1` AS `post1`,`oga`.`phone1` AS `phone1`,`oga`.`mobile1` AS `mobile1`,`oga`.`email1` AS `email1`,`oga`.`name2` AS `name2`,`oga`.`relate2` AS `relate2`,`oga`.`unit2` AS `unit2`,`oga`.`post2` AS `post2`,`oga`.`phone2` AS `phone2`,`oga`.`mobile2` AS `mobile2`,`oga`.`email2` AS `email2`,`oga`.`country` AS `country`,`oga`.`school` AS `school`,`oga`.`start_time` AS `start_time`,`oga`.`end_time` AS `end_time`,`oga`.`type` AS `type`,`oga`.`save_start_time` AS `save_start_time`,`oga`.`save_end_time` AS `save_end_time`,`oga`.`pay_time` AS `pay_time`,`oga`.`status` AS `status`,`oga`.`is_back` AS `is_back`,`oga`.`reason` AS `reason`,`oga`.`create_time` AS `create_time`,`om`.`status` AS `member_status` from (`ow_graduate_abroad` `oga` join `ow_member` `om`) where (`oga`.`user_id` = `om`.`user_id`);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ow_member_stay_view` AS select `oms`.`id` AS `id`,`oms`.`user_id` AS `user_id`,`oms`.`party_id` AS `party_id`,`oms`.`branch_id` AS `branch_id`,`oms`.`country` AS `country`,`oms`.`abroad_time` AS `abroad_time`,`oms`.`return_time` AS `return_time`,`oms`.`pay_time` AS `pay_time`,`oms`.`mobile` AS `mobile`,`oms`.`email` AS `email`,`oms`.`contact_name` AS `contact_name`,`oms`.`contact_mobile` AS `contact_mobile`,`oms`.`status` AS `status`,`oms`.`is_back` AS `is_back`,`oms`.`reason` AS `reason`,`oms`.`apply_time` AS `apply_time`,`om`.`status` AS `member_status` from (`ow_member_stay` `oms` join `ow_member` `om`) where (`oms`.`user_id` = `om`.`user_id`);
