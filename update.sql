...
FROM (
	...
	ALTER ALGORITHM = UNDEFINED DEFINER=`root`@`localhost` VIEW `cadre_inspect_view` AS SELECT `ci`.`id` AS `inspect_id`
...
ALTER TABLE `ext_jzg`
	...
	ADD COLUMN `szzdw2` VARCHAR(255) NULL DEFAULT NULL COMMENT '所在子单位2' AFTER `szzdw1`,
	ADD COLUMN `glqsrq` VARCHAR(255) NULL DEFAULT NULL COMMENT '工龄起始时间' AFTER `szzdw2`,
	ADD COLUMN `jdgl` VARCHAR(255) NULL DEFAULT NULL COMMENT '间断工龄' AFTER `glqsrq`,
	ADD COLUMN `gwzlbm` VARCHAR(255) NULL DEFAULT NULL COMMENT '岗位子类别码' AFTER `jdgl`,
	ADD COLUMN `gwzlbmc` VARCHAR(255) NULL DEFAULT NULL COMMENT '岗位子类别名称' AFTER `gwzlbm`,
	ADD COLUMN `zgdjm` VARCHAR(255) NULL DEFAULT NULL COMMENT '主岗等级码' AFTER `gwzlbmc`,
	ADD COLUMN `zgdjmmc` VARCHAR(255) NULL DEFAULT NULL COMMENT '主岗等级名称' AFTER `zgdjm`,
	ADD COLUMN `zzdjsj` VARCHAR(255) NULL DEFAULT NULL COMMENT '转正定级时间' AFTER `zgdjmmc`,
	ADD COLUMN `zjgwdjm` VARCHAR(255) NULL DEFAULT NULL COMMENT '专技岗位等级码' AFTER `zzdjsj`,
	ADD COLUMN `zjgwdjmc` VARCHAR(255) NULL DEFAULT NULL COMMENT '专技岗位等级名称' AFTER `zjgwdjm`,
	ADD COLUMN `zyjszwpdsj` VARCHAR(255) NULL DEFAULT NULL COMMENT '专业技术职务评定时间' AFTER `zjgwdjmc`,
	ADD COLUMN `zjgwfjsj` VARCHAR(255) NULL DEFAULT NULL COMMENT '专技岗位分级时间' AFTER `zyjszwpdsj`,
	ADD COLUMN `glgwdjm` VARCHAR(255) NULL DEFAULT NULL COMMENT '管理岗位等级码' AFTER `zjgwfjsj`,
	ADD COLUMN `glgwfjsj` VARCHAR(255) NULL DEFAULT NULL COMMENT '管理岗位分级时间' AFTER `glgwdjm`,
	ADD COLUMN `gqgwdjm` VARCHAR(255) NULL DEFAULT NULL COMMENT '工勤岗位等级码' AFTER `glgwfjsj`,
	ADD COLUMN `gqgwfjsj` VARCHAR(255) NULL DEFAULT NULL COMMENT '工勤岗位分级时间' AFTER `gqgwdjm`,
	ADD COLUMN `gqgwdjmc` VARCHAR(255) NULL DEFAULT NULL COMMENT '工勤岗位等级名称' AFTER `gqgwfjsj`;
...
CREATE ALGORITHM = UNDEFINED VIEW `dispatch_view` AS select d.* from dispatch d, dispatch_type dt
    where d.dispatch_type_id = dt.id order by d.year desc, dt.sort_order desc, d.code desc ;

ALTER TABLE `dispatch`
	ADD COLUMN `has_checked` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' COMMENT '是否复核' AFTER `real_dismiss_count`;
...
--2016-9-13
SHOW FIELDS FROM `ext_branch_view`;

CREATE ALGORITHM = UNDEFINED VIEW `ext_branch_view` AS SELECT ob.code as branch_code, ob.name as branch_name, op.name as party_name, u.code as dep_code, u.name as dep_name, count1.member_count, tmp.user_id, tmp.realname,tmp.code from ow_branch ob
left join
(select party_id, branch_id, count(om.user_id) as member_count from ow_member om where branch_id is not null group by party_id, branch_id) count1
on count1.branch_id=ob.id  left join (
select obm.user_id, obm.type_id, ob.id as branch_id, su.realname, su.code from
...
SELECT op.code as branch_code, op.name as branch_name, op.name as party_name, u.code as dep_code, u.name as dep_name, count2.member_count, tmp.user_id, tmp.realname,tmp.code from ow_party op left join (select om.party_id, count(om.user_id) as member_count from ow_member om where om.branch_id is null group by om.party_id) count2 on  count2.party_id=op.id
 left join (
 select opm.user_id, opm.type_id, opmg.party_id, su.realname, su.code  from
ow_party_member opm, ow_party_member_group opmg, base_meta_type bmt, sys_user_view su
 where opmg.is_present=1 and opm.group_id=opmg.id and bmt.code='mt_party_secretary' and opm.type_id = bmt.id and opm.user_id=su.id
 )tmp on tmp.party_id=op.id, unit u, base_meta_type bmt
where op.unit_id=u.id and bmt.code='mt_direct_branch' and op.class_id = bmt.id;
