...
<c:if test="${not empty filePath}">
    ...
    <c:if test="${!fn:endsWith(fn:toLowerCase(filePath), '.pdf')}">
        ...
        <c:if test="${isImage}">
            <a class="various" title="${fileName}" data-path="${cm:sign(filePath)}"
               data-fancybox-type="image" href="${ctx}/pic?path=${cm:sign(filePath)}">${empty label?"查看":label}</a>
        </c:if>
        <c:if test="${!isImage}">
            <a class="downloadBtn" data-type="download" href="javascript:;"
                               data-url="${ctx}/attach_download?path=${cm:sign(filePath)}&filename=${fileName}">${empty label?"下载":label}</a>
        </c:if>
    </c:if>
</c:if>