...
<c:if test="${not empty filePath}">
    <c:if test="${fn:endsWith(filePath, '.pdf')}"><a href="javascript:;" class="popupBtn" data-url="${ctx}/swf/preview?path=${cm:encodeURI(filePath)}&filename=${cm:encodeURI(fileName)}&np=${np}&nd=${nd}">${empty label?"查看":label}</a></c:if>
    <c:if test="${!fn:endsWith(filePath, '.pdf')}"><a class="various" title="${fileName}" data-path="${filePath}" data-fancybox-type="image" href="${ctx}/pic?path=${filePath}">${empty label?"查看":label}</a></c:if>
</c:if>