...
<div class="row">
    <div class="col-xs-12">
        <div id="body-content" class="rownumbers"
            ...
            <c:set var="_query"
                   value="${not empty param.year ||not empty param.partyId || not empty param.code || not empty param.sort}"/>
            <jsp:include page="/WEB-INF/jsp/member/memberReport/menu.jsp"/>
            <div class="space-4"></div>
            <div class="jqgrid-vertical-offset buttons">
                <shiro:hasPermission name="partyReport:edit">
...
<script>
    $("#jqGrid").jqGrid({
        ...
        colModel: [
                ...
                label: '报送', name: 'status', formatter: function (cellvalue, options, rowObject) {
                    ...
                    if (cellvalue == '<%=OwConstants.OW_REPORT_STATUS_UNREPORT%>') {
                        ...
                        return ('<button class="confirm btn btn-success btn-xs" ' +
                            ...
                            'data-url="${ctx}/partyReport_report?id={0}"><i class="fa fa-hand-paper-o"></i> 报送</button>')
                            .format(rowObject.id)
                        </shiro:lacksRole>
                    ...
                    if (cellvalue ==<%=OwConstants.OW_REPORT_STATUS_REPORT%>) {
                        ...
                        return ('<button class="confirm btn btn-danger btn-xs" ' +
                            ...
                            'data-callback="_reload" ' +
                            'data-url="${ctx}/partyReport_report?id={0}&back=1"><i class="fa fa-reply"></i> 退回</button>')
                            .format(rowObject.id)
                        </shiro:hasRole>
                        <shiro:lacksRole name="${ROLE_ODADMIN}">
            ...
            {
                label: '工作总结', name: 'reportFile', formatter: function (cellvalue, options, rowObject) {
                    if (cellvalue == undefined) return '--'
