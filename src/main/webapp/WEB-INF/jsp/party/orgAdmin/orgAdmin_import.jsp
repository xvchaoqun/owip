...
<c:set value="<%=OwConstants.OW_ORG_ADMIN_BRANCH%>" var="OW_ORG_ADMIN_BRANCH"/>
  ...
  <div class="modal-body">
    <form class="form-horizontal" autocomplete="off" disableautocomplete id="modalForm" enctype="multipart/form-data" action="${ctx}org/orgAdmin_import" method="post">
        ...
        <div class="well">
            <c:if test="${param.type==OW_ORG_ADMIN_PARTY}">
                <span class="help-inline">导入的文件请严格按照<a href="${ctx}/attach?code=sample_party_admin">基层党组织管理员录入样表.xlsx</a>（点击下载）的数据格式</span>
            </c:if>
            <c:if test="${param.type==OW_ORG_ADMIN_BRANCH}">
                <span class="help-inline">导入的文件请严格按照<a href="${ctx}/attach?code=sample_branch_admin">党支部管理员录入样表.xlsx</a>（点击下载）的数据格式</span>
            </c:if>
        </div>
  </div>
