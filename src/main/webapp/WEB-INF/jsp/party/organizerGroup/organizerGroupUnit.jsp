...
<div class="modal-header">
    ...
    <h3>${organizerGroup.name}-联系单位</h3>
</div>
<div class="modal-body">
<shiro:hasPermission name="organizerGroup:edit">
    <form class="form-inline" action="${ctx}/organizerGroupUnit" autocomplete="off" disableautocomplete id="modalForm" method="post">
        <div class="form-group">
            <input type="hidden" name="groupId" value="${organizerGroup.id}">
            <select data-rel="select2-ajax" data-ajax-url="${ctx}/unit_selects"
                    name="unitId" data-placeholder="请选择联系单位">
                <option></option>
            </select>
        </div>
    ...
    <div class="popTableDiv"
         data-url-page="${ctx}/organizerGroupUnit?groupId=${organizerGroup.id}"
         data-url-del="${ctx}/organizerGroupUnit_del"
         data-url-co="${ctx}/organizerGroupUnit_changeOrder">
        <c:if test="${commonList.recNum>0}">
            <table class="table table-actived table-striped table-bordered table-hover">
                ...
                <tbody>
                <c:forEach items="${records}" var="groupUnit" varStatus="st">
                    <tr>
                        <c:set var="unit" value="${unitMap.get(groupUnit.unitId)}"/>
                        ...
                        <c:if test="${commonList.recNum>1}">
                            <td nowrap>
                                <a href="javascript:;"
                                   ...
                                   class="changeOrderBtn" data-id="${groupUnit.id}"
                                   data-callback="_updateGroup"
                                   data-direction="1" title="上升"><i
                                ...
                                <a href="javascript:;"
                                   ...
                                   class="changeOrderBtn" data-id="${groupUnit.id}"
                                   data-callback="_updateGroup" data-direction="-1"
                            ...
                            <div class="hidden-md hidden-lg">
                                <div class="inline pos-rel">
                                    ...
                                    <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                        <shiro:hasPermission name="organizerGroup:del">
                                            <li>
                                                <a href="javascript:;" data-id="${groupUnit.id}" class="delBtn tooltip-error" data-rel="tooltip" title="删除">
                                                    <span class="red">
                    ...
                    <c:if test="${!empty commonList && commonList.pageNum>1 }">
                        <wo:page commonList="${commonList}" uri="${ctx}/organizerGroupUnit" target="#modal .modal-content" pageNum="5"
                                 model="3"/>
                    </c:if>
        </c:if>
        <c:if test="${commonList.recNum==0}">
...
<script>
    ...
    $("#modalForm").validate({
        submitHandler: function (form) {
            ...
            //console.log(unitId)
            if(unitId==''){
                $.tip({$target:$("#submitBtn", form), at:"top center",my:"bottom center", msg:"请选择单位"});
                return;
            }
            $(form).ajaxSubmit({
