...
<c:set var="OW_ORGANIZER_TYPE_BRANCH" value="<%=OwConstants.OW_ORGANIZER_TYPE_BRANCH%>"/>
  ...
  <div class="modal-body">
    <form class="form-horizontal" autocomplete="off" disableautocomplete id="modalForm"
          enctype="multipart/form-data" action="${ctx}/organizer_import?cls=${cls}&type=${type}" method="post">
        ...
        <span class="help-inline">导入的文件请严格按照
            <c:if test="${type==OW_ORGANIZER_TYPE_SCHOOL}">
                <a href="${ctx}/attach?code=sample_ow_schoolOrganizer"></c:if>
            <c:if test="${type==OW_ORGANIZER_TYPE_UNIT}">
                <a href="${ctx}/attach?code=sample_ow_unitOrganizer"></c:if>
            <c:if test="${type==OW_ORGANIZER_TYPE_BRANCH}">
                <a href="${ctx}/attach?code=sample_ow_branchOrganizer"></c:if>
                    ${OW_ORGANIZER_TYPE_MAP.get(type)}信息录入样表.xlsx</a>（点击下载）的数据格式</span>
        </div>
  </div>
