    ...
    $("#jqGrid_current").jqGrid({
        ...
        colModel: [
                { label: '规程类型',name: 'type',width: 150,formatter:function(cellvalue, options, rowObject){
                    ...
                    if (cellvalue == <%=CgConstants.CG_RULE_TYPE_STAFF%>) return "组成原则";
                ...
                { label: '相关文件',name: 'filePath',formatter: function (cellvalue, options, rowObject) {
                    ...
                    var fileName = null;
                    if (rowObject.type == <%=CgConstants.CG_RULE_TYPE_STAFF%>) fileName = "组成原则";
                    if (rowObject.type == <%=CgConstants.CG_RULE_TYPE_JOB%>) fileName = "工作职责";
                    if (rowObject.type == <%=CgConstants.CG_RULE_TYPE_DEBATE%>) fileName = "议事规则";
