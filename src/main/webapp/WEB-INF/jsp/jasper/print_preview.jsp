...
<div class="row passport_apply">
    <div class="preview">
        <c:if test="${param.type==JASPER_PRINT_TYPE_LETTER_PRINT}">
            <c:set var="url" value="${ctx}/report/letter_print?ids=${cm:sign(param.ids)}"/>
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_LETTER_FILL_PRINT}">
            <c:set var="url" value="${ctx}/report/letter_fill_print?ids=${cm:sign(param.ids)}"/>
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_MEMBER_STAY_ABROAD}">
            <c:set var="url" value="${ctx}/report/member_stay?type=${MEMBER_STAY_TYPE_ABROAD}&ids=${cm:sign(param.ids)}"/>
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_MEMBER_STAY_INTERNAL}">
            <c:set var="url" value="${ctx}/report/member_stay?type=${MEMBER_STAY_TYPE_INTERNAL}&ids=${cm:sign(param.ids)}"/>
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_MEMBER_CERTIFY}">
            <c:set var="url" value="${ctx}/report/member_certify?type=${JASPER_PRINT_TYPE_MEMBER_CERTIFY}&ids=${cm:sign(param.ids)}"/>
        </c:if>
...
<script>
    $("#print").click(function () {
        <c:if test="${param.type==JASPER_PRINT_TYPE_LETTER_PRINT}">
        $.print("${ctx}/report/letter_print?print=1&ids=${cm:sign(param.ids)}");
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_LETTER_FILL_PRINT}">
        $.print("${ctx}/report/letter_fill_print?print=1&ids=${cm:sign(param.ids)}&type=1");
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_MEMBER_STAY_ABROAD}">
        $.print("${ctx}/report/member_stay?print=1&type=${MEMBER_STAY_TYPE_ABROAD}&ids=${cm:sign(param.ids)}");
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_MEMBER_STAY_INTERNAL}">
        $.print("${ctx}/report/member_stay?print=1&type=${MEMBER_STAY_TYPE_INTERNAL}&ids=${cm:sign(param.ids)}");
        </c:if>
        <c:if test="${param.type==JASPER_PRINT_TYPE_MEMBER_CERTIFY}">
        $.print("${ctx}/report/member_certify?print=1&type=${JASPER_PRINT_TYPE_MEMBER_CERTIFY}&ids=${cm:sign(param.ids)}");
        </c:if>
    });
</script>