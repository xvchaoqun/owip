...
<div class="modal-body">
  <form class="form-horizontal" action="${ctx}/dr/drOnline_inspectorNotice" autocomplete="off" disableautocomplete id="modalForm" method="post">
    ...
    <div class="form-group">
      <div class="col-xs-6">
        ...
        <textarea id="contentId">
             <c:if test="${drOnline!=null}">${drOnline.inspectorNotice}</c:if>
        </textarea>
      </div>
    </div>
  </form>
...
<script>
  ...
  $("select[name=noticeName]").on("change",function(){
    ...
    if (noticeId.length == 0) {
      if (dr.inspectorNotice != undefined) {
        ke.html(KindEditor.unescape(dr.inspectorNotice))
      }
    }
