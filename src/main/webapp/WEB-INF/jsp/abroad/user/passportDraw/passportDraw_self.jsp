    ...
    <div class="widget-main no-padding">
            ...
            <div class="row my_paginate_row">
              ...
              <div class="col-xs-6">
                <div class="my_paginate">
                  <ul class="pagination">
                    <wo:page commonList="${commonList}" uri="${ctx}/user/abroad/passportDraw_self" target="#page-content" pageNum="5"
                             model="3"/>
                  </ul>
                </div>
              </div>
        ...
        <c:if test="${commonList.recNum==0}">
          <div class="well well-lg center">
            <h4 class="green lighter">没有未出行的因私出国申请<c:if test="${param.auth!='admin'}">，<a href="${ctx}/user/abroad/applySelf">现在去申请</a></c:if></h4>
          </div>
        </c:if>
    </div><!-- /.widget-main -->
  </div><!-- /.widget-body -->
...
<div class="modal-footer center" style="margin-top: 20px">
  ...
  <c:if test="${param.auth!='admin'}">
    <input data-url="${ctx}/user/abroad/passportDraw_select" class="openView  btn btn-default" value="返回"/>
  </c:if>
