...
<%@ include file="/WEB-INF/jsp/common/taglibs.jsp"%>         
  ...
  <div class="modal-body">
    <form class="form-horizontal" autocomplete="off" disableautocomplete id="modalForm" enctype="multipart/form-data"
          action="${ctx}/abroad/passportApply_import" method="post">
        ...
        <div class="well">
        <span class="help-inline">导入的文件请严格按照<a href="${ctx}/attach?code=sample_passportApply">办理证件记录录入样表.xlsx</a>（点击下载）的数据格式</span>
        </div>
  </div>
  <div class="modal-footer">
      <span class="note">
          <ul>
              <li>导入后的证件，保存至【批准办理新证件（已交证件）】中</li>
          </ul>
      </span>
					...
					$(form).ajaxSubmit({
						...
						success:function(ret){
							if(ret && ret.addCount>=0){
								...
								SysMsg.success(result.format(ret.total, ret.addCount, ret.total-ret.addCount), '成功',function(){
									$.loadPage({url:"${ctx}/abroad/passportApply?status=3"})
								});
							}
							$btn.button('reset');
						}
