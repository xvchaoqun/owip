...
<%@ include file="/WEB-INF/jsp/abroad/constants.jsp" %>
  <div class="modal-header">
    ...
    <h4>
<c:if test="${param.type==ABROAD_APPROVER_TYPE_UNIT}">"本单位正职"</c:if>
<c:if test="${param.type==ABROAD_APPROVER_TYPE_LEADER}">"分管校领导"</c:if>
<c:if test="${param.type!=ABROAD_APPROVER_TYPE_UNIT && param.type!=ABROAD_APPROVER_TYPE_LEADER}">"${approverType.name}"</c:if>
		所包含的干部
    </h4>
  </div>
