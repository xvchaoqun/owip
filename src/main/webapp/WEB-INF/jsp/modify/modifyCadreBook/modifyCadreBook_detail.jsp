    ...
    <div class="widget-body">
        <div class="widget-main">
            <table class="table  table-unhover table-bordered table-striped">
                ...
                <tr>
                    <td data-code="pubTime">出版日期</td>
                    <td class="bg-left">${cm:formatDate(modify.pubTime,'yyyy-MM-dd')}</td>
                    <td data-code="name">著作名称</td>
                    <td class="bg-left">${modify.name}</td>
                    <td data-code="publisher">出版社</td>
                    <td class="bg-left">${modify.publisher}</td>
                </tr>
                <tr>
                    <td data-code="type">类型</td>
                    <td class="bg-left">${CADRE_BOOK_TYPE_MAP.get(modify.type)}</td>
...
<div class="clearfix form-actions center">
    ...
    && mta.type != MODIFY_TABLE_APPLY_TYPE_DELETE}">
        <button class="popupBtn btn btn-primary" ${mta.status!=MODIFY_TABLE_APPLY_STATUS_APPLY?'disabled':''}
                data-url="${ctx}/cadreBook_au?toApply=1&cadreId=${cadre.id}&_isUpdate=1&opType=${param.opType}&id=${modify.id}&applyId=${mta.id}"
                type="button">
...
<script>
    ...
    $("#jqGrid_records").jqGrid({
        ...
        url: '${ctx}/cadreBook_data?cadreId=${cadre.id}&${cm:encodeQueryString(pageContext.request.queryString)}',
        multiselect: false,
