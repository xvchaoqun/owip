        ...
        <div class="widget-main">
            <table class="table  table-unhover table-bordered table-striped" width="500">
                ...
                <tr>
                    ...
                    <td data-code="gradeTime">工勤岗位分级时间</td>
                    <td colspan="3" class="bg-left">${cm:formatDate(modify.gradeTime,'yyyy.MM')}</td>
                </tr>
                <tr>
                    <td data-code="isCurrent">是否当前工勤岗位</td>
...
<div class="clearfix form-actions center">
    ...
    && mta.type != MODIFY_TABLE_APPLY_TYPE_DELETE}">
        <button class="popupBtn btn btn-primary" ${mta.status!=MODIFY_TABLE_APPLY_STATUS_APPLY?'disabled':''}
                data-url="${ctx}/cadrePostWork_au?toApply=1&cadreId=${cadre.id}&_isUpdate=1&opType=${param.opType}&id=${modify.id}&applyId=${mta.id}"
                type="button">
...
<script>
    ...
    $("#jqGrid_records").jqGrid({
        ...
        url: '${ctx}/cadrePostWork_data?cadreId=${cadre.id}&${cm:encodeQueryString(pageContext.request.queryString)}',
        multiselect: false,
