    ...
    <div class="widget-body">
        <div class="widget-main">
            <table class="table  table-unhover table-bordered table-striped">
                <tr>
                    <td data-code="type" width="100">类型</td>
                    <td class="bg-left" width="200">${CADRE_COURSE_TYPE_MAP.get(modify.type)}</td>
                    <td data-code="name" width="100">课程名称</td>
                    <td class="bg-left" width="200">${modify.name}</td>
...
<div class="clearfix form-actions center">
    ...
    && mta.type != MODIFY_TABLE_APPLY_TYPE_DELETE}">
        <button class="popupBtn btn btn-primary" ${mta.status!=MODIFY_TABLE_APPLY_STATUS_APPLY?'disabled':''}
                data-url="${ctx}/cadreCourse_au?toApply=1&cadreId=${cadre.id}&_isUpdate=1&opType=${param.opType}&id=${modify.id}&applyId=${mta.id}"
                type="button">
