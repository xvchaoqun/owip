    ...
    <div class="widget-body">
        <div class="widget-main">
            <table class="table  table-unhover table-bordered table-striped">
                <tr>
                    <td data-code="isCurrent">是否当前管理岗位</td>
                    <td  colspan="5" class="bg-left">${modify.isCurrent?'是':'否'}</td>
                </tr>
                <tr>
                    <td data-code="level">管理岗位等级</td>
                    <td class="bg-left">${cm:getMetaType(modify.level).name}</td>
                    <td data-code="gradeTime">管理岗位分级时间</td>
...
<div class="clearfix form-actions center">
    ...
    && mta.type != MODIFY_TABLE_APPLY_TYPE_DELETE}">
        <button class="popupBtn btn btn-primary" ${mta.status!=MODIFY_TABLE_APPLY_STATUS_APPLY?'disabled':''}
                data-url="${ctx}/cadrePostAdmin_au?toApply=1&cadreId=${cadre.id}&_isUpdate=1&opType=${param.opType}&id=${modify.id}&applyId=${mta.id}"
                type="button">
...
<script>
    ...
    $("#jqGrid_records").jqGrid({
        ...
        url: '${ctx}/cadrePostAdmin_data?cadreId=${cadre.id}&${cm:encodeQueryString(pageContext.request.queryString)}',
        multiselect: false,
