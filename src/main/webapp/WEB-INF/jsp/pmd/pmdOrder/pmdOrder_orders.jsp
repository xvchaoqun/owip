...
<div class="modal-body">
    <table class="table table-striped table-bordered table-condensed table-unhover2">
        ...
        <tbody>
        <c:forEach items="${pmdOrders}" var="o" varStatus="vs">
            <tr>
                <td>${o.sn}</td>
                <td>${o.payer}</td>
                <td>${o.payername}</td>
...
<script>
    function _closeTrade(sn){
        $.post("${ctx}/pmd/pmdOrder_closeTrade", {sn:sn},function(data){
            if(data.success){
                $.loadModal("${ctx}/pmd/pmdOrder_orders?memberId=${param.memberId}", "850");
            }
        });
    }
</script>