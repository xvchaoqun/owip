...
<div class="row">
    <div class="col-xs-12">
        ...
        <div id="body-content" class="myTableDiv"
             data-url-page="${ctx}/cla/claApply"
             data-url-export="${ctx}/cla/claApply_data"
             data-querystr="${cm:encodeQueryString(pageContext.request.queryString)}">
            <c:set var="_query" value="${not empty param.cadreId ||not empty param._applyDate
            ||not empty param.type || not empty param.code || not empty param.isModify || not empty param.isBack || not empty param.sort}"/>
            <div class="tabbable">
                <ul class="nav nav-tabs padding-12 tab-color-blue background-blue">
                    <li class="<c:if test="${status==0}">active</c:if>">
                        <a href="javascript:;" class="loadPage" data-url="${ctx}/cla/claApply?status=0"><i
                                class="fa fa-circle-o"></i> 干部请假申请</a>
                    </li>
                    <li class="<c:if test="${status==1}">active</c:if>">
                        <a href="javascript:;" class="loadPage" data-url="${ctx}/cla/claApply?status=1"><i
                                class="fa fa-check"></i> 同意申请</a>
                    </li>
                    <li class="<c:if test="${status==2}">active</c:if>">
                        <a href="javascript:;" class="loadPage" data-url="${ctx}/cla/claApply?status=2"><i
                    ...
                    <div class="buttons pull-right" style="top: -3px; right:10px; position: relative">
                        <a class="openView btn btn-success btn-sm"
                           data-url="${ctx}/htmlFragment_au?editContent=no&code=hf_cla_apply_note"><i
                                class="fa fa-plus"></i> 申请说明</a>
                        <a class="openView btn btn-primary btn-sm"
                           data-url="${ctx}/htmlFragment_au?editContent=no&code=hf_cla_apply_approval_note"><i
                ...
                <div class="tab-content">
                    <div class="tab-pane in active">
                        <div class="jqgrid-vertical-offset buttons">
                            <c:if test="${status==0}">
                                <button data-url="${ctx}/cla/claApply_au"
                            ...
                            <c:if test="${status==0||status==1}">
                                <shiro:hasPermission name="claApply:edit">
                                    <button class="jqOpenViewBtn btn btn-success btn-sm"
                                            data-url="${ctx}/cla/claApply_change" data-open-by="page">
                            ...
                            <button class="jqOpenViewBtn btn btn-info btn-sm"
                                    ...
                                    data-querystr="&type=<%=SystemConstants.SYS_APPROVAL_LOG_TYPE_CLA_APPLY%>"
                                    data-open-by="page">
                            ...
                            <c:if test="${status==1}">
                                <button class="jqOpenViewBtn btn btn-success btn-sm"
                                        data-url="${ctx}/cla/claApply_back" data-id-name="applyId">
                                    <i class="fa fa-pencil-square-o"></i> 销假
                                </button>
                                </c:if>
                            <c:if test="${status>=0 && status!=1}">
                            ...
                            <c:if test="${status==-1}">
                                <shiro:hasPermission name="claApply:del">
                                    <a class="jqBatchBtn btn btn-success btn-sm"
                                       data-url="${ctx}/cla/claApply_batchUnDel" data-title="找回已删除干部请假申请"
                                       data-msg="确定恢复这{0}条申请记录吗？"><i class="fa fa-reply"></i> 恢复申请</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="passportApply:del">
                                    <a class="jqBatchBtn btn btn-danger btn-sm"
                                       data-url="${ctx}/cla/claApply_doBatchDel" data-title="删除申请"
                                ...
                                <div class="widget-main no-padding">
                                    <form class="form-inline search-form" id="searchForm">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <select name="isModify" data-rel="select2"
                                                        data-width="100"
                                                        data-placeholder="请选择">
                                                    <option></option>
                                        ...
                                        <div class="form-group">
                                            <label>是否销假</label>
                                            <div class="input-group">
                                                <select name="isBack" data-rel="select2"
                                                        data-width="100"
                                                        data-placeholder="请选择">
                                                    <option></option>
                                                    <option value="0">否</option>
...
<script>
    $("#detailBtn").click(function () {
        ...
        var url = "${ctx}/cla/claApply_view?id=" + id;
    ...
    $("#jqGrid").jqGrid({
        ...
        url: '${ctx}/cla/claApply_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [
            <c:if test="${status==0}">
            {
            ...
            <c:if test="${status!=0}">
            {label: '销假情况', name: 'isBack', formatter: $.jgrid.formatter.TRUEFALSE,
        ...
        rowattr: function (rowData, currentObj, rowId) {
            ...
            if (tdType != 1 && applyId > 0) {
                $.getJSON("${ctx}/cla/claApply_approvers", {
                    applyId: applyId,
