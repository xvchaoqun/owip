...
<div class="modal-body">
     <form class="form-horizontal" action="${ctx}/pcs/pcsPrParty_candidate_import_confirm" autocomplete="off"
    ...
    <table class="table table-bordered table-striped">
        ...
        <tr data-user-id="${can.userId}" data-type="${can.type}"
            data-branch-vote="${can.branchVote}" data-vote="${can.vote}" data-positive-vote="${can.positiveVote}">
            <td>
                ...
                <select required name="userId${vs.index}" data-rel="select2-ajax"
                        data-ajax-url="${ctx}/member_selects?noAuth=1&politicalStatus=<%=MemberConstants.MEMBER_POLITICAL_STATUS_POSITIVE%>&status=${MEMBER_STATUS_NORMAL},${MEMBER_STATUS_TRANSFER}"
            ...
            <td>${can.realname}</td>
            <td>${PCS_PR_USER_TYPE_MAP.get(can.type)}</td>
            <td>${can.vote}</td>
    ...
    submitHandler: function (form) {
        ...
        $.each($("#modalForm tbody tr"), function (i, tr) {
            ...
            var item ={userId:userId, type:$(tr).data("type"),
                branchVote:$(tr).data("branch-vote"),
        ...
        $(form).ajaxSubmit({
            data: {items: $.base64.encode(JSON.stringify(items)), stage:${stage}},
...
function _importUser(candidate) {
  ...
  var $select = $("#select" + candidate.type);
