        ...
        <div id="body-content" class="rownumbers"
            ...
            <ul class="nav nav-tabs padding-12 tab-color-blue background-blue">
                <li class="<c:if test="${!isQuit}">active</c:if>">
                    <a href="javascript:;" class="loadPage" data-url="${ctx}/pcsCommitteeMember?type=${type?1:0}&isQuit=0">
                        <i class="fa fa-circle-o-notch fa-spin"></i> 现任${!type?'党委委员':'纪委委员'}</a>
                </li>
                <li class="<c:if test="${isQuit}">active</c:if>">
                    <a href="javascript:;" class="loadPage" data-url="${ctx}/pcsCommitteeMember?type=${type?1:0}&isQuit=1">
                        <i class="fa fa-history"></i> 离任${!type?'党委委员':'纪委委员'}</a>
                </li>
            </ul>
            ...
            <div class="jqgrid-vertical-offset buttons">
                <shiro:hasPermission name="pcsCommitteeMember:edit">
                    <button class="popupBtn btn btn-success btn-sm"
                            data-url="${ctx}/pcsCommitteeMember_au?type=${type?1:0}&isQuit=${isQuit?1:0}">
                ...
                <shiro:hasPermission name="cadre:leave">
                    ...
                    <button class="jqOpenViewBtn btn btn-warning btn-sm"
                            data-url="${ctx}/pcsCommitteeMember_leave?type=${type?1:0}">
                    ...
                    <button class="jqItemBtn btn btn-success btn-sm"
                            data-msg="确认返回现任库？"
                            data-querystr="type=${type?1:0}&isQuit=0"
                            data-url="${ctx}/pcsCommitteeMember_leave">
                        <i class="fa fa-reply"></i> 返回现任库
                    </button>
                    </c:if>
                </shiro:hasPermission>

                <shiro:hasPermission name="pcsCommitteeMember:del">
                    <button data-url="${ctx}/pcsCommitteeMember_batchDel"
                            data-title="删除"
                        ...
                        <form class="form-inline search-form" id="searchForm">
                            <div class="form-group">
                                ...
                                <select required data-rel="select2" name="post"
                                    ...
                                    <option></option>
                                    <jsp:include
                                            page="/metaTypes?__code=${type?'mc_committee_member_jw':'mc_committee_member_dw'}"/>
                                </select>
                                <script>
                            ...
                            <div class="clearfix form-actions center">
                                <a class="jqSearchBtn btn btn-default btn-sm"
                                   data-url="${ctx}/pcsCommitteeMember?type=${type?1:0}&isQuit=${isQuit?1:0}"
...
<script>
    ...
    $("#jqGrid").jqGrid({
        rownumbers: true,
        url: '${ctx}/pcsCommitteeMember_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [
            ...
            <c:if test="${!_query}">
            {
                ...
                formatoptions: {url: '${ctx}/pcsCommitteeMember_changeOrder'}, frozen: true
            },
            </c:if>
