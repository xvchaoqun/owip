...
<%@ include file="/WEB-INF/jsp/pcs/constants.jsp" %>
  ...
  <div class="modal-body">
    <form class="form-horizontal" autocomplete="off" disableautocomplete id="modalForm" enctype="multipart/form-data"
		  action="${ctx}/pcs/pcsPollReport_import" method="post">
        <input type="hidden" name="pollId" value="${param.pollId}"/>
        <div class="form-group">
            <label class="col-xs-3 control-label"><span class="star">*</span> 候选人类型</label>
            <div class="col-xs-6">
                <select required data-rel="select2" name="type" data-width="270"
                        data-placeholder="请选择">
                    <option></option>
                    <c:forEach items="${PCS_USER_TYPE_MAP}" var="entry" end="${stage==PCS_POLL_THIRD_STAGE?1:2}">
        ...
        <div class="well">
        <span class="help-inline">导入的文件请严格按照<a href="${ctx}/attach?code=sample_pcsPollReport">党代会候选人名单录入样表.xlsx</a>（点击下载）的数据格式</span>
        </div>
  </div>
