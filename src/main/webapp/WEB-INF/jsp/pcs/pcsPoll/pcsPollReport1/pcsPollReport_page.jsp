                    ...
                    <shiro:hasPermission name="pcsPollReport:edit">
                        <c:if test="${!pcsPoll.hasReport}">
                            ...
                            <button class="popupBtn btn btn-primary btn-sm tooltip-success"
                                    data-url="${ctx}/pcs/pcsPollReport_import?pollId=${param.pollId}"
                                    data-rel="tooltip" data-placement="top" title="批量导入"><i class="fa fa-upload"></i> 导入候选人名单</button>
                        </c:if>
                        <button class="jqExportBtn btn btn-success btn-sm tooltip-success"
                                    data-url="${ctx}/pcs/pcsPollReport_data?pollId=${param.pollId}"
                                    data-grid-id="#jqGrid2"
                        ...
                        <a class="popupBtn btn btn-success btn-sm"
                           data-rel="tooltip" data-placement="bottom" title="可查询所有教职工和学生的信息"
                           data-url="${ctx}/pcs/user_search"><i class="fa fa-search"></i> 候选人学工号查询</a>
                    </shiro:hasPermission>
                            ...
                            <div class="form-group">
                                ...
                                <div class="clearfix form-actions center">
                                    <a class="jqSearchBtn btn btn-default btn-sm"
                                       data-url="${ctx}/pcs/pcsPollReportList?pollId=${param.pollId}&type=${type}"
...
<script>
    function _cancelReload(){
        SysMsg.success('取消成功。',function(){
            $("#body-content-view").loadPage("${ctx}/pcs/pcsPollReportList?pollId=${param.pollId}&type=${type}");
        })
    }
    ...
    $("#changeType input[type=checkbox]").click(function () {
        $("#body-content-view").loadPage("${ctx}/pcs/pcsPollReportList?pollId=${param.pollId}&type="+$(this).val());
    })
</script>