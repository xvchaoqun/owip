...
<%@ include file="/WEB-INF/jsp/parttime/constants.jsp" %>
  <div class="modal-header">
    ...
    <h4>
<c:if test="${param.type==PARTTIME_APPROVER_TYPE_UNIT}">"院系党组织"</c:if>
<c:if test="${param.type==PARTTIME_APPROVER_TYPE_LEADER}">"分管校领导"</c:if>
<c:if test="${param.type!=PARTTIME_APPROVER_TYPE_UNIT && param.type!=PARTTIME_APPROVER_TYPE_LEADER}">"${approverType.name}"</c:if>
		所包含的干部
    </h4>
  </div>
  <div class="modal-body">
  <form class="form-horizontal"  action="${ctx}/parttimeApproverType/selectCadres" autocomplete="off" disableautocomplete id="modalForm" method="post">
  ...
  <script>
	...
	$(function(){
		$.getJSON("${ctx}/parttimeApproverType/selectCadres_tree",{id:"${param.id}", type:"${param.type}"},function(data){
		...
		$("#modal form").validate({
				...
				submitHandler: function (form) {
					<c:if test="${param.type!=PARTTIME_APPROVER_TYPE_UNIT && param.type!=PARTTIME_APPROVER_TYPE_LEADER}">
