...
<div class="modal-header">
    ...
    <h3><c:if test="${dpWork!=null}">编辑</c:if><c:if test="${dpWork==null}">添加</c:if><c:if test="${not empty fid}">其间</c:if>工作经历
...
<div class="modal-body overflow-visible">
    <form class="form-horizontal" action="${ctx}/dp/dpWork_au?userId=${sysUser.userId}" autocomplete="off" disableautocomplete id="modalForm" method="post">
            <input type="hidden" name="id" value="${dpWork.id}">
				...
				<div class="col-xs-6">
                    <div class="input-group date" data-date-min-view-mode="1"
                        ...
                        <input required class="form-control" name="_startTime" type="text"
                                placeholder="yyyy.mm" value="${cm:formatDate(dpWork.startTime,'yyyy.MM')}" />
				...
				<div class="col-xs-7">
                    <div class="input-group date" style="width: 120px"
                        ...
                        <input ${not empty topEndTime?"required":""} placeholder="yyyy.mm"
                           ...
                           value="${cm:formatDate(dpWork.endTime,'yyyy.MM')}"/>
                        <span class="input-group-addon"> <i class="fa fa-calendar bigger-110"></i></span>
                    </div>
                <c:if test="${not empty fid}">
			...
			<div class="form-group">
				...
				<div class="col-xs-6">
                    <textarea required class="form-control" type="text" name="detail" >${dpWork.detail}</textarea>
				</div>
			</div>
			<div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-6">
                <label>
                    <input name="isCadre" ${dpWork.isCadre?"checked":""}  type="checkbox" />
                    <span class="lbl"></span>
                </label>
            </div>
        </div>
			...
			<div class="form-group">
				...
				<div class="col-xs-6">
                        <textarea class="form-control limited" type="text" name="remark" >${dpWork.remark}</textarea>
				</div>
			</div>
    </form>
</div>
<div class="modal-footer">
   <a href="javascript:;" data-dismiss="modal" class="btn btn-default">取消</a>
    <input type="submit" class="btn btn-primary" value="<c:if test="${dpWork!=null}">确定</c:if><c:if test="${dpWork==null}">添加</c:if>"/>
</div>
<script>
