...
<div class="modal-body">
    <form class="form-horizontal" action="${ctx}/dp/dpNpr_cancel" autocomplete="off" disableautocomplete id="modalForm" method="post">
        <input type="hidden" name="ids[]" value="${param['ids[]']}">
		<c:set var="count" value="${fn:length(fn:split(param['ids[]'],\",\"))}"/>
		<c:if test="${count>=1}">
			<div class="form-group">
				<label class="col-xs-3 control-label">移除党外代表人士</label>
...
<div class="modal-footer">
    ...
    <button id="submitBtn"
            ...
            class="btn btn-primary"><i class="fa fa-check"></i> ${not empty dpNpr?'确定':'移除'}</button>
</div>
<script>
