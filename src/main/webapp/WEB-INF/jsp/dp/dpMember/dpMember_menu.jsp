...
<ul class="nav nav-tabs padding-12 tab-color-blue background-blue">
  ...
  <a href="javascript:;" class="loadPage" data-url="${ctx}/dp/dpMember?cls=1"}><i class="fa fa-th${cls==1?'-large':''}"></i> 学生民主党派成员(${cm:trimToZero(student_normalCount)})</a>
  </li>
  <li class="${cls==2?'active':''}">
  <a href="javascript:;" class="loadPage" data-url="${ctx}/dp/dpMember?cls=2"}><i class="fa fa-th${cls==2?'-large':''}"></i> 在职民主党派成员(${cm:trimToZero(teacher_normalCount)})</a>
  </li>
  <li class="${cls==3?'active':''}">
  <a href="javascript:;" class="loadPage" data-url="${ctx}/dp/dpMember?cls=3"}><i class="fa fa-th${cls==3?'-large':''}"></i> 离退休民主党派成员(${cm:trimToZero(teacher_retireCount)})</a>
  </li>
  ...
  <li class="dropdown <c:if test="${cls==6||cls==7}">active</c:if>" >
    <a data-toggle="dropdown" class="dropdown-toggle" href="javascript:;">
      <i class="fa fa-sign-out"></i> 已转出民主党派成员${cls==6?"(学生)":(cls==7)?"(教工)":""}
  ...
  <div class="buttons hidden-sm hidden-xs" style="padding-left:10px; position: relative">
    ...
    <shiro:hasAnyRoles name="${ROLE_ADMIN},${ROLE_ODADMIN},${ROLE_DPPARTYADMIN}">
    <a class="popupBtn btn btn-danger btn-sm"
       data-rel="tooltip" data-placement="bottom" title="可查询所有教职工和学生的组织关系状态"
            data-url="${ctx}/dp/dpMember/search"><i class="fa fa-search"></i> 全校组织关系查询</a>
    </shiro:hasAnyRoles>
