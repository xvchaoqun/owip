...
<div class="row">
    <div class="col-xs-12">
        <div id="body-content" class="rownumbers" data-querystr="${cm:encodeQueryString(pageContext.request.queryString)}">
            <c:set var="_query" value="${not empty param.year ||not empty param.type ||not empty param.status ||not empty param.userId ||not empty param.partyId || not empty param.code || not empty param.sort}"/>
...
<script>
    $("#jqGrid").jqGrid({
        ...
        colModel: [
                { label: '年度',name: 'year'},
                { label: '类型',name: 'type'},
                { label: '组织员',name: 'userId'},
                { label: '联系党委',name: 'partyId'},
                { label: '联系单位',name: 'units'},
                { label: '任职时间',name: 'appointDate'},
                { label: '离任时间',name: 'dismissDate'},
