...
<div class="row">
    <div class="col-xs-12">
        <div id="body-content" class="rownumbers" data-querystr="${cm:encodeQueryString(pageContext.request.queryString)}">
            <c:set var="_query" value="${not empty param.userId ||not empty param.post ||not empty param.isCadre}"/>
            <div class="jqgrid-vertical-offset buttons">
                <shiro:hasPermission name="sp:edit">
                    <button class="popupBtn btn btn-info btn-sm"
                            data-url="${ctx}/sp/spCg_au?type=${type}"><i class="fa fa-plus"></i>
                        添加</button>
                    <button class="jqOpenViewBtn btn btn-primary btn-sm"
                            data-url="${ctx}/sp/spCg_au"
                            data-grid-id="#jqGrid"><i class="fa fa-edit"></i>
                        修改</button>
                    <shiro:hasPermission name="cg:menu">
                        <button class="popupBtn btn btn-warning btn-sm"
                                data-url="${ctx}/sp/spCg_relevance?type=${type}"><i class="fa fa-random"></i>
                            关联</button>
                    </shiro:hasPermission>
                    <button data-url="${ctx}/sp/spCg_batchDel"
                            data-title="删除"
...
<script>
    $("#jqGrid").jqGrid({
        rownumbers:true,
        url: '${ctx}/sp/spCg_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [

                { label: '职务',name: 'post',width: 150,frozen: true},
                { label: '席位',name: 'seat',width: 300,frozen: true},
                { label: '工作证号',name: 'user.code',width: 150,frozen: true},
                { label: '姓名',name: 'user.realname',frozen: true},
                { label: '性别',name: 'user.gender',width: 80,formatter: $.jgrid.formatter.GENDER,frozen: true},
                { label: '排序', width: 80, formatter: $.jgrid.formatter.sortOrder,
                formatoptions:{url: "${ctx}/sp/spCg_changeOrder"}, frozen: true},
                { label: '所在单位',name: 'unitId',width: 250,formatter: $.jgrid.formatter.unit},
                ...
                { label: '管理岗位等级',name: 'manageLevel',width: 200},
                { label: '是否领导干部',name: 'isCadre',formatter: $.jgrid.formatter.TRUEFALSE},
                { label: '所担任行政职务',name: 'adminPost',width: 300,align: 'left'},
