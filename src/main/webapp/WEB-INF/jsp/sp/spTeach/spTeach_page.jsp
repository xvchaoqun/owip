...
<div class="row">
    <div class="col-xs-12">
        <div id="body-content" class="rownumbers" data-querystr="${cm:encodeQueryString(pageContext.request.queryString)}">
            <c:set var="_query" value="${not empty param.post ||not empty param.userId ||not empty param.isCadre || not empty param.code || not empty param.sort}"/>
            <div class="jqgrid-vertical-offset buttons">
                <shiro:hasPermission name="sp:edit">
                    <button class="popupBtn btn btn-info btn-sm"
                            data-url="${ctx}/sp/spTeach_au">
                        <i class="fa fa-plus"></i> 添加</button>
                    <button class="jqOpenViewBtn btn btn-primary btn-sm"
                       data-url="${ctx}/sp/spTeach_au"
                       data-grid-id="#jqGrid"><i class="fa fa-edit"></i>
                        修改</button>
                    <button data-url="${ctx}/sp/spTeach_batchDel"
                            data-title="删除"
...
<script>
    $("#jqGrid").jqGrid({
        rownumbers:true,
        url: '${ctx}/sp/spTeach_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [
                { label: '姓名',name: 'user.realname',frozen: true},
                { label: '工作证号',name: 'user.code',width: 150,frozen: true},
                { label: '界数',name: 'th',frozen: true},
                { label: '职务',name: 'post',width: 200,formatter: $.jgrid.formatter.MetaType,frozen: true},
            <shiro:hasPermission name="sp:edit">
                { label: '排序', width: 80, formatter: $.jgrid.formatter.sortOrder,
                formatoptions:{url: "${ctx}/sp/spTeach_changeOrder"},frozen: true},
            </shiro:hasPermission>
                ...
                { label: '专业技术职务',name: 'proPost',align: 'lift',width: 200},
                { label: '是否领导干部',name: 'isCadre',formatter: $.jgrid.formatter.TRUEFALSE},
                { label: '所担任行政职务',name: 'adminPost',align: 'lift',width: 300},
