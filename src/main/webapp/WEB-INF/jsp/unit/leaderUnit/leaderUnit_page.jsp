...
<div class="row">
    <div class="col-xs-12">
        ...
        <div class="myTableDiv"
             data-url-au="${ctx}/leaderUnit_au"
             data-url-page="${ctx}/leaderUnit_page"
            ...
            <c:if test="${commonList.recNum>0}">
                <table class="table table-actived table-striped table-bordered table-hover table-condensed">
                    ...
                    <tr>
                        <th class="center">
							...
							<th>所属单位</th>
							<th>类别</th>
                        <shiro:hasPermission name="leaderUnit:changeOrder">
                    ...
                    <c:forEach items="${leaderUnits}" var="leaderUnit" varStatus="st">
                        <tr>
                            <td class="center">
                                <label class="pos-rel">
                                    <input type="checkbox" value="${leaderUnit.id}" class="ace">
                                    <span class="lbl"></span>
                                </label>
                            </td>
								<td nowrap>${leaderUnit.leaderId}</td>
								<td nowrap>${leaderUnit.unitId}</td>
								<td nowrap>${leaderUnit.typeId}</td>
                            <shiro:hasPermission name="leaderUnit:changeOrder">
                            <c:if test="${!_query && commonList.recNum>1}">
                                <td nowrap>
                                    <a href="#" <c:if test="${commonList.pageNo==1 && st.first}">style="visibility: hidden"</c:if> class="changeOrderBtn" data-id="${leaderUnit.id}" data-direction="1" title="上升"><i class="fa fa-arrow-up"></i></a>
                                    <input type="text" value="1"
                                           class="order-step tooltip-success" data-rel="tooltip" data-placement="top" title="修改操作步长">
                                    <a href="#" <c:if test="${commonList.pageNo>=commonList.pageNum && st.last}">style="visibility: hidden"</c:if> class="changeOrderBtn" data-id="${leaderUnit.id}" data-direction="-1" title="下降"><i class="fa fa-arrow-down"></i></a>                                </td>
                                </td>
                            </c:if>
                            </shiro:hasPermission>
                            <td nowrap>
                                <div class="hidden-sm hidden-xs action-buttons">
                                    <shiro:hasPermission name="leaderUnit:edit">
                                    <button data-id="${leaderUnit.id}" class="editBtn btn btn-default btn-mini btn-xs">
                                ...
                                <div class="hidden-md hidden-lg">
                                    <div class="inline pos-rel">
                                        ...
                                        <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                            ...
                                            <shiro:hasPermission name="leaderUnit:edit">
                                            <li>
                                                <a href="#" data-id="${leaderUnit.id}" class="editBtn tooltip-success" data-rel="tooltip" title="编辑">
                                                    <span class="green">
                ...
                <wo:page commonList="${commonList}" uri="${ctx}/leaderUnit_page" target="#page-content" pageNum="5"
                         model="3"/>
            </c:if>
            <c:if test="${commonList.recNum==0}">
