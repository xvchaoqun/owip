...
<script>
  var colModel = [
    ...
    { label: '单位编号', name: 'unitCode', width: 80,frozen:true },
    { label: '单位名称', name: 'unitName', width: 350, align:'left', formatter:function(cellvalue, options, rowObject){
      return '<a href="javascript:;" class="openView" data-url="${ctx}/unit_view?id={0}">{1}</a>'
              .format(rowObject.unitId, cellvalue);
    },frozen:true },
    { label: '单位类型', name: 'unitTypeId', width: 250,frozen:true, formatter: $.jgrid.formatter.MetaType },
    </c:if>
    { label: '岗位编号',name: 'code', frozen:true},
    { label: '岗位名称',name: 'name', align:'left', width: 150, frozen:true},
