...
<div class="modal-header">
    ...
    <h3>${dispatchUnit.dispatch.code}-关联单位</h3>
</div>
            ...
            <table class="table table-actived table-striped table-bordered table-hover">
                ...
                <c:forEach items="${dispatchUnitRelates}" var="dispatchUnitRelate" varStatus="st">
                        ...
                        <td nowrap>
                            <div class="hidden-sm hidden-xs action-buttons">
                                <shiro:hasPermission name="dispatchUnitRelate:del">
                                    <button class="delBtn btn btn-danger btn-xs" data-id="${dispatchUnitRelate.id}">
                    ...
                    <c:if test="${!empty commonList && commonList.pageNum>1 }">
                        <wo:page commonList="${commonList}" uri="${ctx}/dispatchUnitRelate" target="#modal .modal-content" pageNum="5"
                                 model="3"/>
                    </c:if>
        </c:if>
        <c:if test="${commonList.recNum==0}">
