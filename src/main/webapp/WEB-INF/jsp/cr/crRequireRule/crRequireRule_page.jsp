    ...
    <div class="widget-header">
        ...
        <div class="widget-toolbar no-border">
            <ul class="nav nav-tabs">
                <li class="active">
                    <a href="javascript:;">${crRequire.name}-岗位招聘规则</a>
                </li>
            </ul>
        </div>
    </div>
...
<script type="text/template" id="subgrid_op_tpl">
    <button class="popupBtn btn btn-xs btn-primary"
            data-url="${ctx}/crRuleItem_au?id={{=id}}&&fid={{=parentRowKey}}"><i
...
<script>
    ...
    $("#jqGrid2").jqGrid({
        //forceFit:true,
        pager: "jqGridPager2",
        url: '${ctx}/crRequireRule_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [
            {
            ...
            }, width: 90},
            {
                ...
                formatoptions:{grid:'#jqGrid2', url: "${ctx}/crRequireRule_changeOrder"}, frozen: true
            },
            ...
            {label: '具体条件', name: '_items', width: 90, formatter: function (cellvalue, options, rowObject) {
                ...
                return '<button class="popupBtn btn btn-success btn-xs" data-width="700" data-url="${ctx}/crRuleItem_au?requireRuleId={0}"><i class="fa fa-plus"></i> 添加</button>'
                        .format(rowObject.id);
            }},
    ...
    function subGridRowExpanded(parentRowID, parentRowKey) {
        ...
        var childGridURL = '${ctx}/crRuleItem_data?requireRuleId={0}&${cm:encodeQueryString(pageContext.request.queryString)}'.format(parentRowKey);
        ...
        $("#" + childGridID).jqGrid({
            ...
            colModel: [
                ...
                {
                    label: '排序', width: 80, formatter: $.jgrid.formatter.sortOrder,
                    formatoptions:{grid:'#jqGrid2', url: "${ctx}/crRuleItem_changeOrder"}, frozen: true
                },
