...
<div class="jqgrid-vertical-offset buttons">
    <shiro:hasPermission name="cetSpecialPlan:edit">
        <button class="popupBtn btn btn-info btn-sm"
                data-url="${ctx}/cet/cetSpecialPlan_au?specialId=${param.specialId}">
            <i class="fa fa-plus"></i> 添加
        </button>
        <button class="jqOpenViewBtn btn btn-primary btn-sm"
                data-url="${ctx}/cet/cetSpecialPlan_au"
                data-grid-id="#jqGrid"><i class="fa fa-edit"></i>
            修改
        </button>
    </shiro:hasPermission>
    <shiro:hasPermission name="cetSpecialPlan:del">
        <button data-url="${ctx}/cet/cetSpecialPlan_batchDel"
                data-title="删除"
    ...
    <%--<button class="jqExportBtn btn btn-success btn-sm tooltip-success"
            data-url="${ctx}/cet/cetSpecialPlan_data"
...
<script>
    $("#jqGrid2").jqGrid({
        rownumbers:true,
        pager: "jqGridPager2",
        url: '${ctx}/cet/cetSpecialPlan_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [
            ...
            {label: '培训内容', name: '_summary', width: 80, formatter: function (cellvalue, options, rowObject) {
                ...
                'data-url="${ctx}/cet/cetSpecialPlan_summary?id={0}"><i class="fa {3}"></i> {1}</button>')
