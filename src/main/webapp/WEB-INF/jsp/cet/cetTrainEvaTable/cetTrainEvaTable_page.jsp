...
<div class="row">
    <div class="col-xs-12">
        ...
        <div id="body-content" class="myTableDiv"
                 data-url-page="${ctx}/cet/cetTrainEvaTable"
                 data-url-export="${ctx}/cet/cetTrainEvaTable_data"
                 data-querystr="${cm:encodeQueryString(pageContext.request.queryString)}">
            <c:set var="_query" value="${not empty param.name || not empty param.code || not empty param.sort}"/>
            <div class="jqgrid-vertical-offset buttons">
                <shiro:hasPermission name="cetTrainEvaTable:edit">
                    <a class="popupBtn btn btn-info btn-sm"  data-url="${ctx}/cet/cetTrainEvaTable_au"><i class="fa fa-plus"></i> 添加</a>
                    <a class="jqOpenViewBtn btn btn-primary btn-sm"
                       data-url="${ctx}/cet/cetTrainEvaTable_au"
                       data-grid-id="#jqGrid"
                       data-querystr="&"><i class="fa fa-edit"></i>
                        修改</a>
                </shiro:hasPermission>
                <shiro:hasPermission name="cetTrainEvaTable:del">
                    <button data-url="${ctx}/cet/cetTrainEvaTable_batchDel"
                            data-title="删除"
...
<script>
    $("#jqGrid").jqGrid({
        url: '${ctx}/cet/cetTrainEvaTable_data?callback=?&${cm:encodeQueryString(pageContext.request.queryString)}',
        colModel: [
...

            {
                ...
                return _.template($("#sort_tpl").html().NoMultiSpace())({id: rowObject.id, url:"${ctx}/cet/cetTrainEvaTable_changeOrder"})
            }, frozen: true
            },
            {label: '评估指标', name: 'normNum', formatter: function (cellvalue, options, rowObject) {
                ...
                if (num==0)
                    return '<a href="javascript:void(0)" class="openView" data-url="${ctx}/cet/cetTrainEvaNorm?evaTableId={0}">编辑指标</a>'
                            .format(rowObject.id);
                else
                    return '<a href="javascript:void(0)" class="openView" data-url="${ctx}/cet/cetTrainEvaNorm?evaTableId={0}">查看指标（{1}）</a>'
            ...
            {label: '评估等级', name: 'rankNum', formatter: function (cellvalue, options, rowObject) {
                ...
                if(cellvalue!=undefined) num=cellvalue;
                if (num==0)
                    return '<a href="javascript:void(0)" class="openView" data-url="${ctx}/cet/cetTrainEvaRank?evaTableId={0}">编辑评估等级</a>'
                            .format(rowObject.id);
                else
                    return '<a href="javascript:void(0)" class="openView" data-url="${ctx}/cet/cetTrainEvaRank?evaTableId={0}">查看评估等级（{1}）</a>'
                            .format(rowObject.id, cellvalue);
            }, width: 200},
