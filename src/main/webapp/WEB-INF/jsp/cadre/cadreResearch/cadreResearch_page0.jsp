                        ...
                        <tr>
                            ...
                            <td>
                                <div class="hidden-sm hidden-xs action-buttons">
                                    <shiro:hasPermission name="cadreResearch:edit">
                                    <button onclick="_au(${cadreResearch.id})" class="btn btn-default btn-mini btn-xs">
                                        <i class="fa fa-edit"></i> 编辑
                                    </button>
                                     </shiro:hasPermission>
                                     <shiro:hasPermission name="cadreResearch:del">
                                    <button class="btn btn-danger btn-mini btn-xs" onclick="_del(${cadreResearch.id})">
...
<script>
    ...
    function cadreReward_au(id) {
        url = "${ctx}/cadreReward_au?rewardType=${CADRE_REWARD_TYPE_RESEARCH}&cadreId=${param.cadreId}";
        if (id > 0)  url += "&id=" + id;
        loadModal(url);
    }
    ...
    function _reload(){
        $("#modal").modal('hide');
        $("#view-box .tab-content").load("${ctx}/cadreResearch_page?${cm:encodeQueryString(pageContext.request.queryString)}");
    }
