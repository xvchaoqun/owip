                        ...
                        <tr>
                            ...
                            <td>
                                <div class="hidden-sm hidden-xs action-buttons">
                                    <shiro:hasPermission name="cadreReward:edit">
                                    <button onclick="_au(${cadreReward.id})" class="btn btn-default btn-xs">
                                        <i class="fa fa-edit"></i> 编辑
                                    </button>
                                     </shiro:hasPermission>
                                     <shiro:hasPermission name="cadreReward:del">
                                    <button class="btn btn-danger btn-xs" onclick="_del(${cadreReward.id})">
...
<script>
    function _au(id) {
        url = "${ctx}/cadreReward_au?rewardType=${param.rewardType}&cadreId=${param.cadreId}";
        if (id > 0)  url += "&id=" + id;
        loadModal(url);
    }
...
function _reload(){
    $("#modal").modal('hide');
    $("#view-box .tab-content").load("${ctx}/cadreReward_page?${cm:encodeQueryString(pageContext.request.queryString)}");
}
