            ...
            $(form).ajaxSubmit({
                success:function(ret){
                    if(ret.success){
                        ...
                        $("#item-content").load("${ctx}/modifyCadreTrain_detail?applyId=${param.applyId}&_="+new Date().getTime())
                        </c:if>
                        <c:if test="${param._isUpdate!=1}">
                        location.href='?cls=1&module=${MODIFY_TABLE_APPLY_MODULE_CADRE_TRAIN}';
                        </c:if>
                        </c:if>
                    }
                }
