...
<%@ include file="/WEB-INF/jsp/common/taglibs.jsp"%>         
  <div class="modal-header">
    ...
    <h3>导入干部任免审批表(word文档)</h3>
  </div>
  <div class="modal-body">
    <form class="form-horizontal" autocomplete="off" disableautocomplete
          id="modalForm" enctype="multipart/form-data" action="${ctx}/cadreAdform_docxImport" method="post">
		<div class="form-group">
			<label class="col-xs-4 text-right">单个干部任免审批表文件</label>
			<div class="col-xs-6">
				<input type="file" name="docx" extension="docx"/>
			</div>
		</div>
		<div class="form-group">
        ...
        <div class="well" style="margin-bottom: 0">
            <ul>
                <li>如需批量上传，请先将word格式的任免审批表打包成zip文件</li>
  ...
  <script>
	  ...
	  $.fileInput($('#modalForm input[name=docx]'),{
            allowExt: ['docx']
        })
	  $.fileInput($('#modalForm input[name=zip]'),{
		...
		$("#modalForm").validate({
				...
				submitHandler: function (form) {
				    ...
				    if($('#modalForm input[name=docx]').val()=='' && $('#modalForm input[name=zip]')==''){
				        return;
                    }
