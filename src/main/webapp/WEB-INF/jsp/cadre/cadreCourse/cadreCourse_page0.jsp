...
<script>
    ...
    function cadreReward_au(id) {
        url = "${ctx}/cadreReward_au?rewardType=${CADRE_REWARD_TYPE_TEACH}&cadreId=${param.cadreId}";
        if (id > 0)  url += "&id=" + id;
        loadModal(url);
    }
    ...
    function _reload(){
        $("#modal").modal('hide');
        $("#view-box .tab-content").load("${ctx}/cadreCourse_page?${cm:encodeQueryString(pageContext.request.queryString)}");
    }
</script>