...
<script>
    ...
    colModels.cadre = [
        ...
        {
            label: '学校类型', name: 'schoolType', width: 80, formatter: function (cellvalue, options, rowObject) {
            if (cellvalue == undefined) return '--';
        ...
        },*/
        {
                ...
                if ($.trim(pdfFilePath) != '') {
                    ...
                    ret = '<button href="javascript:void(0)" data-url="${ctx}/pdf_preview?path={0}&filename={1}"  title="PDF文件预览" class="popupBtn btn btn-xs btn-primary"><i class="fa fa-search"></i> 预览</button>'
                            ...
                            + '&nbsp;<button data-url="${ctx}/cadre_download?path={0}&filename={1}&cadreId={2}" title="下载PDF文件" class="downloadBtn btn btn-xs btn-warning"><i class="fa fa-file-pdf-o"></i> PDF</button>'
                                    .format(encodeURI(pdfFilePath), encodeURI(fileName), rowObject.cadreId);
                }
                ...
                if ($.trim(wordFilePath) != '') {
                    ...
                    ret += '&nbsp;<button data-url="${ctx}/cadre_download?path={0}&filename={1}&cadreId={2}"  title="下载WORD文件" class="downloadBtn btn btn-xs btn-success"><i class="fa fa-file-word-o"></i> DOC</button>'
                            .format(encodeURI(wordFilePath), encodeURI(fileName), rowObject.cadreId);
                }
                return ret;
            }
        },
