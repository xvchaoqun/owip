...
<ul class="jqgrid-vertical-offset nav nav-tabs padding-12 tab-color-blue background-blue">
    <li class="${type==1?"active":""}">
        <a href="javascript:;" onclick="_innerPage(1)"><i class="fa fa-list"></i> 兼职情况</a>
    </li>
...
<c:if test="${type==1}">
    ...
    <c:if test="${hasDirectModifyCadreAuth}">
            <c:if test="${!cm:getHtmlFragment('hf_cadre_parttime').isDeleted}">
                ...
                <div class="widget-body">
                    <div class="widget-main" style="min-height: 647px" id="orginal">
                        <jsp:useBean id='map' class='java.util.HashMap' scope='request'>
                            <c:set target='${map}' property='cadreParttimes' value='${cadreParttimes}'/>
                        </jsp:useBean>
                        ${cm:freemarker(map, '/cadre/cadreParttime.ftl')}
                    </div>
                </div>
            </div>
        </div>
...
<c:if test="${type==1}">
    <script>
        ...
        function _innerPage(type, fn) {
            $("#tab-content").loadPage({url:"${ctx}/cadreParttime_page?cadreId=${param.cadreId}&type=" + type, callback:fn})
        }
