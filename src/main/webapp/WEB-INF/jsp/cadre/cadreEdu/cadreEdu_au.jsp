				...
				success:function(ret){
					if(ret.success){
						...
						<c:if test="${param.toApply==1}">
							...
							$("#body-content-view").load("${ctx}/modifyTableApply_detail?module=${MODIFY_TABLE_APPLY_MODULE_CADRE_EDU}&applyId=${param.applyId}&_="+new Date().getTime())
							</c:if>
							<c:if test="${param._isUpdate!=1}">
							$.hashchange('cls=1&module=${MODIFY_TABLE_APPLY_MODULE_CADRE_EDU}');
							</c:if>
						</c:if>
					}
				}
