    ...
    <div class="col-xs-12">
        <c:set var="_query" value="${not empty param.year || not empty param.name || not empty param.title || not empty param.unitId || not empty param.cadreId}"/>
            <div class="jqgrid-vertical-offset buttons">
                ...
                <input type="hidden" name="_auth" value="${param._auth}">
                <shiro:hasPermission name="cesResult:edit">
                    <button class="popupBtn btn btn-info btn-sm"
                            data-url="${ctx}/cesResult_au?cadreId=${param.cadreId}&unitId=${param.unitId}&type=${param.type}&_auth=${param._auth}">
                        <i class="fa fa-plus"></i> 添加</button>
                    <button class="jqOpenViewBtn btn btn-primary btn-sm"
                       data-url="${ctx}/cesResult_au?cadreId=${param.cadreId}&unitId=${param.unitId}&type=${param.type}&_auth=${param._auth}"
                       data-grid-id="#jqGrid_evaResult"><i class="fa fa-edit"></i>
                        修改</button>
                    <button class="popupBtn btn btn-info btn-sm tooltip-info" data-url="/cesResult_import_page?type=${param.type}" data-rel="tooltip" data-placement="top" title="批量导入"><i class="fa fa-upload"></i>
                        批量导入
                    </button>
                </shiro:hasPermission>
            ...
            <div class="widget-body">
                <div class="widget-main no-padding">
                    <form class="form-inline search-form" id="searchForm">
                        ...
                        <div class="form-group">
                            <label>测评类别</label>
                            <input class="form-control search-query" name="name" type="text" value="${param.groupName}"
                                   placeholder="请输入测评类别">
                        </div>
                        <div class="form-group">
