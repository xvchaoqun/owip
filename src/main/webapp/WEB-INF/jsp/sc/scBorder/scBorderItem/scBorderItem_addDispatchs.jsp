                ...
                <tr>
                    ...
                    <td nowrap><c:if test="${not empty dispatch.fileName}">
                        <a href="javascript:;" data-type="download"
                                       data-url="${ctx}/attach_download?path=${cm:sing(dispatch.file)}&filename=${dispatch.fileName}"
...
<shiro:hasPermission name="${PERMISSION_CADREADMIN}">
    ...
    <c:if test="${type!='edit'}">
        <button class="popupBtn btn btn-success"
                data-url="${ctx}/sc/scBorderItem_addDispatchs?id=${param.id}&cadreId=${param.cadreId}&type=edit"
    ...
    <script>
        function addDispatch(){
            ...
            $.post("${ctx}/sc/scBorderItem_addDispatchs",{id:'${param.id}',ids:ids},function(ret){
                if(ret.success) {
                    $("#modal").modal('hide');
                    _reload2();
                }
