...
<script>
    function _reuseScRecord(){
        ...
        $.post("${ctx}/sc/scMatterCheckItem_reuse?itemId=${smci.id}", {recordIds: ids} ,function (ret) {
            if(ret.success){
                $("#modal").modal('hide');
    ...
    var selectedRecords = ${cm:toJSONArray(smci.scRecords)}
    $("#jqGrid_popup").jqGrid({
        pager:null,
        rownumbers: true,
        loadui:'disabled',
        width:1020,
        height:270,
        url: '${ctx}/sc/scRecords?userId=${smci.userId}&year=${smci.year}',
        colModel: [
            {label: '年份', name: 'year', width: 80, frozen: true},
            { label:'工作证号', name: '_code',formatter: function (){return '${smci.code}'}},
            { label:'考察对象', name: '_realname',formatter: function (){return '${smci.realname}'}},
