...
<script>
  var colModel = [
      ...
      { label: '领表时间',name: 'drawTime', width: 150, formatter: 'date', formatoptions: {srcformat:'Y-m-d H:i',newformat:'Y-m-d H:i'} },
      { label: '应交回时间',name: 'handTime',cellattr:function(rowId, val, rowObject, cm, rdata) {
          ...
          if ($.trim(rowObject.realHandTime)=='' && _date <= new Date().format('yyyy-MM-dd hh:mm'))
              return "class='danger'";
      }, width: 150, formatter: 'date', formatoptions: {srcformat:'Y-m-d H:i',newformat:'Y-m-d H:i'}},
      {label: '实交回日期', name: 'realHandTime', formatter: 'date', formatoptions: {newformat: 'Y-m-d'}},
      {label: '封面填表日期', name: 'fillTime', formatter: 'date', formatoptions: {newformat: 'Y-m-d'}},
