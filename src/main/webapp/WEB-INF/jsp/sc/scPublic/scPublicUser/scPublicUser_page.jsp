            ...
            }},
            {
                ...
                if ($.trim(pdfFilePath) != '') {
                    ...
                    ret = '<button href="javascript:void(0)" data-url="${ctx}/pdf_preview?path={0}&filename={1}"  title="PDF文件预览" class="popupBtn btn btn-xs btn-primary"><i class="fa fa-search"></i> 预览</button>'
                            ...
                            + '&nbsp;<button data-url="${ctx}/sc/scPublicUser_download?id={0}&fileType=1" title="下载PDF文件" class="downloadBtn btn btn-xs btn-warning"><i class="fa fa-file-pdf-o"></i> PDF</button>'
                                    .format(rowObject.id/*encodeURI(pdfFilePath), encodeURI(_num)*/);
                }
                var wordFilePath = rowObject.wordFilePath;
                if ($.trim(wordFilePath) != '') {

                    ret += '&nbsp;<button data-url="${ctx}/sc/scPublicUser_download?id={0}&fileType=2"  title="下载WORD文件" class="downloadBtn btn btn-xs btn-success"><i class="fa fa-file-word-o"></i> DOC</button>'
                            .format(rowObject.id/*encodeURI(wordFilePath), _num*/);
                }
                return ret;
            }},
