...
public class ApprovalLogController extends BaseController {
    ...
    public Map do_approvalLog_au(ApprovalLog record, HttpServletRequest request) {
        ...
        if (id == null) {
            ...
            logger.info(addLog(request, SystemConstants.LOG_ABROAD, "添加因私出国审批记录：%s", record.getId()));
        } else {

            approvalLogService.updateByPrimaryKeySelective(record);
            logger.info(addLog(request, SystemConstants.LOG_ABROAD, "更新因私出国审批记录：%s", record.getId()));
        }
    ...
    public Map do_approvalLog_del(HttpServletRequest request, Integer id) {
        ...
        if (id != null) {
            ...
            logger.info(addLog(request, SystemConstants.LOG_ABROAD, "删除因私出国审批记录：%s", id));
        }
        return success(FormUtils.SUCCESS);
    }
    ...
    public Map batchDel(HttpServletRequest request, @RequestParam(value = "ids[]") Integer[] ids, ModelMap modelMap) {
        ...
        if (null != ids && ids.length>0){
            ...
            logger.info(addLog(request, SystemConstants.LOG_ABROAD, "批量删除因私出国审批记录：%s", StringUtils.join(ids, ",")));
        }
