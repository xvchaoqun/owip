...
@Controller
public class MemberInflowController extends MemberBaseController {
    ...
    public Map do_memberInflow_au(@CurrentUser SysUserView loginUser,MemberInflow record,
        ...
        if (id == null) {
            ...
            applyApprovalLogService.add(record.getId(),
                    ...
                    "提交流入党员申请");
            logger.info(addLog(LogConstants.LOG_MEMBER, "添加流入党员：%s", record.getId()));
        } else {

            memberInflowService.updateByPrimaryKeySelective(record);
            logger.info(addLog(LogConstants.LOG_MEMBER, "更新流入党员：%s", record.getId()));
        }
    ...
    public Map do_memberInflow_del(HttpServletRequest request, Integer id) {
        ...
        if (id != null) {
            ...
            logger.info(addLog(LogConstants.LOG_MEMBER, "删除流入党员：%s", id));
        }
        return success(FormUtils.SUCCESS);
    }*/
    ...
    public Map batchDel(HttpServletRequest request, Integer[] ids, ModelMap modelMap) {
        ...
        if (null != ids && ids.length > 0) {
            ...
            logger.info(addLog(LogConstants.LOG_MEMBER, "批量删除流入党员：%s", StringUtils.join(ids, ",")));
        }
    ...
    public void memberInflow_export(MemberInflowExample example, HttpServletResponse response) {
        ...
        String[] titles = {"学工号|100","姓名|100", "所在党组织|300|left","所在党支部|300|left", "原职业|150|left", "流入前所在省份|80",
