...
public class PmMeeting2Controller extends PmBaseController {
    ...
    public Map do_pmMeeting2_au(PmMeeting2 record, Byte[] type,Integer number1 ,String time1,Integer number2 ,String time2, MultipartFile _file, @RequestParam(defaultValue = "0")Byte reedit,    //reedit 重新编辑
        ...
        if (id == null) {
            ...
            logger.info(log( LogConstants.LOG_PM, "添加三会一课2(支部会议)：{0}", record.getId()));
        } else {
            if (reedit == 1) {
    ...
    public void pmMeeting2_export(PmMeeting2Example example, HttpServletResponse response) {
        ...
        for (int i = 0; i < rownum; i++) {
            ...
            String[] values = {
                    ...
                    record.getAddress(),
                    record.getType2()==null?PARTY_MEETING_MAP.get(record.getType1()):PARTY_MEETING_MAP.get(record.getType1())+","+PARTY_MEETING_MAP.get(record.getType2()),
                    record.getNumber2()==null?"第"+record.getNumber1()+"次":"第"+record.getNumber1()+"次"+","+"第"+record.getNumber2()+"次",
                    record.getTime2()==null?record.getTime1()+"分钟":record.getTime1()+"分钟"+","+record.getTime2()+"分钟",
    ...
    public String pmMeeting2_count(Integer year,
                                   Byte quarter,
                                   Integer month,
                                   Integer partyId,
                                   Integer branchId,ModelMap modelMap) {
    ...
    public String pmMeeting2_import(ModelMap modelMap) {
        ...
        return "pm/pmMeeting/pmMeeting2_import";
    }
