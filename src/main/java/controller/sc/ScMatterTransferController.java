...
import domain.sc.ScMatterTransferExample.Criteria;
...
public class ScMatterTransferController extends ScBaseController {
    ...
    public String scMatterTransfer() {

        return "sc/scMatterTransfer/scMatterTransfer_page";
    }
    ...
    public Map do_scMatterTransfer_au(ScMatterTransfer record, HttpServletRequest request) {
        ...
        if (id == null) {
            ...
            logger.info(addLog( SystemConstants.LOG_SC, "添加个人有关事项-移交记录：%s", record.getId()));
        } else {

            scMatterTransferService.updateByPrimaryKeySelective(record);
            logger.info(addLog( SystemConstants.LOG_SC, "更新个人有关事项-移交记录：%s", record.getId()));
        }
    ...
    public String scMatterTransfer_au(Integer id, ModelMap modelMap) {
        ...
        return "sc/scMatterTransfer/scMatterTransfer_au";
    }

    @RequiresPermissions("scMatterTransfer:del")
    @RequestMapping(value = "/scMatterTransfer_del", method = RequestMethod.POST)
    @ResponseBody
    public Map do_scMatterTransfer_del(HttpServletRequest request, Integer id) {

        if (id != null) {

            scMatterTransferService.del(id);
            logger.info(addLog( SystemConstants.LOG_SC, "删除个人有关事项-移交记录：%s", id));
        }
        return success(FormUtils.SUCCESS);
    }
    ...
    public Map batchDel(HttpServletRequest request, @RequestParam(value = "ids[]") Integer[] ids, ModelMap modelMap) {
        ...
        if (null != ids && ids.length>0){
            ...
            logger.info(addLog( SystemConstants.LOG_SC, "批量删除个人有关事项-移交记录：%s", StringUtils.join(ids, ",")));
        }
