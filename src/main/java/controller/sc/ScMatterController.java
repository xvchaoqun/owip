...
import domain.sc.ScMatterExample.Criteria;
...
public class ScMatterController extends ScBaseController {
    ...
    public String scMatter() {

        return "sc/scMatter/scMatter_page";
    }
    ...
    @ResponseBody
    public Map do_scMatter_au(ScMatter record, HttpServletRequest request) {

        Integer id = record.getId();
        if (id == null) {
            scMatterService.insertSelective(record);
            logger.info(addLog( SystemConstants.LOG_SC, "添加个人有关事项：%s", record.getId()));
        } else {

            scMatterService.updateByPrimaryKeySelective(record);
    ...
    public String scMatter_au(Integer id, ModelMap modelMap) {

        if (id != null) {
            ScMatter scMatter = scMatterMapper.selectByPrimaryKey(id);
            modelMap.put("scMatter", scMatter);
        }
        return "sc/scMatter/scMatter_au";
    }

    @RequiresPermissions("scMatter:del")
    @RequestMapping(value = "/scMatter_del", method = RequestMethod.POST)
    @ResponseBody
    public Map do_scMatter_del(HttpServletRequest request, Integer id) {

        if (id != null) {

            scMatterService.del(id);
            logger.info(addLog( SystemConstants.LOG_SC, "删除个人有关事项：%s", id));
        }
        return success(FormUtils.SUCCESS);
    }
