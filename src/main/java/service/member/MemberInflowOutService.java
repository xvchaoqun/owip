...
public class MemberInflowOutService extends MemberBaseMapper {
    ...
    public MemberInflow out(int userId, String outUnit, Integer outLocation, String _outTime, boolean reset){
        ...
        // 2 要求 提出申请并未完成审批的记录
        if(memberInflow.getOutStatus()!=null&&memberInflow.getOutStatus()==MemberConstants.MEMBER_INFLOW_OUT_STATUS_PARTY_VERIFY){
            throw new OpException("状态异常");
        }
    ...
    public void removeInflowMember(int userId, boolean isDirect){
        ...
        MemberInflow record = new MemberInflow();
        record.setId(memberInflow.getId());
        record.setOutStatus(MemberConstants.MEMBER_INFLOW_OUT_STATUS_PARTY_VERIFY);
    ...
    public void memberInflowOut_check(Integer[] ids, byte type, int loginUserId){
        ...
        for (int id : ids) {
            ...
            applyApprovalLogService.add(memberInflow.getId(),
                    ...
                    OwConstants.OW_APPLY_APPROVAL_LOG_TYPE_MEMBER_INFLOW_OUT, (type == 1) ? "支部审核" : "基层党组织审核", (byte) 1, null);
    ...
    public void memberInflowOut_back(Integer[] ids, byte status, String reason, int loginUserId){
        ...
        for (int id : ids) {
            ...
            MemberInflow memberInflow = memberInflowMapper.selectByPrimaryKey(id);
            if(memberInflow.getOutStatus() >= MemberConstants.MEMBER_INFLOW_OUT_STATUS_BRANCH_VERIFY){
                if(!PartyHelper.hasPartyAuth(loginUserId, memberInflow.getPartyId()))
                    throw new UnauthorizedException();
            }
            if(memberInflow.getOutStatus() >= MemberConstants.MEMBER_INFLOW_OUT_STATUS_BACK){
