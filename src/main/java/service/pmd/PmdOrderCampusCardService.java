...
//@Service
public class PmdOrderCampusCardService extends PmdBaseMapper {
    ...
    // 构建支付表单参数
    private PmdOrder confirmOrder(String oldOrderNo, int pmdMemberId, boolean isSelfPay) {
        ...
        params.put("macc", "");
        params.put("commnet", "");
        params.put("sno_id_name", payer);
        newOrder.setParams(JSONUtils.toString(params, false));
    ...
    public void savePayNotify(HttpServletRequest request) {
        ...
        try {
            ...
            record.setSn(request.getParameter("sn"));
            record.setAmt(request.getParameter("amt"));
            record.setParams(JSONUtils.toString(request.getParameterMap(), false));
            record.setIsSuccess(StringUtils.equals(request.getParameter("paid"), "true"));
    ...
    // 服务器通知签名串
    public static String notifySignStr(String paycode, String sn, String amt, String payer, String paid, String paidtime) {
        
        String md5Str = keys + paycode + sn + amt + payer
                + paid + paidtime + StringUtils.reverse(keys);
    ...
    public boolean verifyNotifySign(HttpServletRequest request) {
        ...
        String md5Str = notifySignStr(request.getParameter("paycode"),
                request.getParameter("sn"),
                request.getParameter("amt"), request.getParameter("payer"),
                request.getParameter("paid"), request.getParameter("paidtime"));
        String verifySign = MD5Util.md5Hex(md5Str, "utf-8");
        
        boolean ret = StringUtils.equalsIgnoreCase(request.getParameter("sign"), verifySign);
        if (!ret) {
            logger.warn("签名校验失败，{}, md5Str={}, verifySign={}",
                    JSONUtils.toString(request.getParameterMap(), false), md5Str, verifySign);
        }
    ...
    // 跳转页面前的支付确认，生成支付订单号
    @Transactional
    public PmdOrder payConfirm(int pmdMemberId, boolean isSelfPay) {
        ...
        // 确认订单信息
        PmdOrder pmdOrder = confirmOrder(oldOrderNo, pmdMemberId, isSelfPay);
        ...
        // 检查一下生成的所有订单，是否已经有支付成功的记录，如有则不允许跳转
        {
            ...
            try {
                for (PmdOrder pmdOrder : pmdOrders) {
                    ...
                    sn = pmdOrder.getSn();
                    queryRet = query(sn, pmdOrder.getPayer());
                    ...
                    if (StringUtils.isNotBlank(queryRet)) { // 支付系统查询订单号不存在的话，接口返回的内容为空
                        ...
                        if (paid.getAsBoolean()) {
                            ...
                            throw new OpException("当前缴费记录已由{0}(工号：{1})支付成功，支付订单号{2}，请不要重复支付。",
                                    payUser != null ? payUser.getRealname() : "", code, sn);
                        }
                    }
                }
    ...
    public void processCallback(HttpServletRequest request) {
        ...
        try {
            // 签名校验成功 且 确认交易成功
            if (verifyNotifySign(request) && StringUtils.equals(request.getParameter("paid"), "true")) {
                ...
                if (pmdOrder == null) {
                    ...
                    if (isBatch) {
                        processBatchOrder(orderNo, payer, new BigDecimal(amt));
                    } else {
                        
                        processSingleOrder(pmdOrder.getMemberId(), orderNo,
                                payer, new BigDecimal(amt));
                    }
                }
            }
        } catch (Exception ex) {
    ...
    // 单个缴费通知处理
    private void processSingleOrder(int memberId, String orderNo, String payer, BigDecimal realPay) {
    ...
    // 处理批量缴费结果通知
    private void processBatchOrder(String orderNo, String payer, BigDecimal realPay) {
        ...
        for (PmdOrderItem pmdOrderItem : pmdOrderItems) {
            ...
            processSingleOrder(pmdOrderItem.getMemberId(), orderNo, payer, pmdOrderItem.getDuePay());
        }
    ...
    */
    @Transactional
    public CloseTradeRet closeTrade(String sn) {
       ...
       /* PmdOrder record = new PmdOrder();
        ...
        try {
            ...
            if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                } else if (code != null && StringUtils.contains(code.getAsString(), "2002")) {
                    
                    logger.error("关闭订单{}失败，接口响应内容：{}", sn, ret);
                    throw new OpException("关闭订单{0}失败，该订单已支付成功，请确认。", sn);
                } else {
                    logger.warn("关闭订单{}失败，接口响应内容：{}", sn, ret);
                }
                
                closeTradeRet.ret = ret;
            }
        } catch (IOException ex) {
            
            logger.error("关闭订单{}失败", sn, ex);
            throw new OpException("关闭订单{0}失败：{1}。", sn, ex.getMessage());
        }
