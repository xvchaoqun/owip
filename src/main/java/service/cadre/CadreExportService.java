...
public class CadreExportService extends BaseMapper {
    ...
    public List<String> getTitles(){
        ...
        return new ArrayList<>(Arrays.asList(new String[]{
                /*1*/"工作证号|100", "姓名|100", cadreStateName + "|100", "部门属性|150",
                /*5*/"所在单位|300", "所在单位及职务|300", "行政级别|100", "职务属性|100",
                /*9*/"是否正职|120",  "是否班子负责人|120", "性别|50", "民族|100", "籍贯|100", "出生地|100",
                /*15*/"户籍地|100", "身份证号|150", "出生时间|100", "年龄|50", "政治面貌|150",
                /*20*/"党派加入时间|120", "参加工作时间|120", "到校日期|100", "最高学历|120", "最高学位|120",
                /*25*/"毕业时间|100", "学习方式|120", "毕业学校|200", "学校类型|100", "所学专业|200",
                /*30*/"全日制教育学历学位|100", "全日制教育毕业院校系及专业|400", "在职教育学历学位|100", "在职教育毕业院系及专业|400", "最高毕业院校及专业|400",
                /*35*/"专业技术职务|150", "专技职务评定时间|200", "现职务任命文件|150",
                /*38*/"任现职时间|100", "现职务始任时间|150", "现职务始任年限|120", "现职级始任时间|150", "任现职级年限|120",
                /*43*/"兼任单位及职务|250", "兼任职务现任时间|180", "兼任职务始任时间|150", "是否双肩挑|100", "双肩挑单位|100",
                /*48*/"联系方式|100", "办公电话|100", "电子信箱|200", "所在党组织|500",
                 /*52*/"是否有挂职经历|100", "备注|500","学习经历|500|left","工作经历|500|left","简历|500|left","年度考核结果|300|left"}));
    }
    ...
    public SXSSFWorkbook export(Byte status, CadreViewExample example, int exportType, Integer[] cols, int isReserve) {
        ...
        int[] exportCloumns_1 = new int[]{1, 2, 4, 5, 6, 7, 8, 10,11,12,13,15, 16, 17, 18, 19, 20, 23, 35,
                38,39,40,41,42,48,50};
        if (exportType == 1) {
            //新增一个角色，限制查看干部库权限，
            // 字段为：1工作证号，姓名，5部门属性，所在单位、7所在单位及职务、行政级别、职务属性，11是否班子负责人、12性别、13民族、14籍贯
            // 16身份证号、出生时间、18年龄、党派、党派加入时间、23最高学历、
            // 34专业技术职务、37任现职时间、联系方式、电子邮箱。
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (fulltimeEdu != null) {
                ...
                if(StringUtils.isNotEmpty(degree)){
                    _fulltimeEdu += (StringUtils.isNotBlank(_fulltimeEdu)?"\n":"") + degree;
                }
            ...
            if (onjobEdu != null) {
                ...
                if(StringUtils.isNotEmpty(degree)){
                    _onjobEdu += (StringUtils.isNotBlank(_onjobEdu)?"\n":"") + degree;
                }
