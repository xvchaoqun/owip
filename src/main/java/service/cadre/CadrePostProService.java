...
public class CadrePostProService extends BaseMapper {
    ...
    public void updateModify(CadrePostPro record, Integer applyId) {
        ...
        record.setId(null);
        record.setStatus(null);
        if (cadrePostProMapper.updateByExampleSelective(record, example) > 0 && mta.getUserId().intValue() == currentUserId) {
    ...
    @Transactional
    public void modifyApply(CadrePostPro record, Integer id, boolean isDelete, String reason) {
    ...
    public ModifyTableApply approval(ModifyTableApply mta, ModifyTableApply record, Boolean status) {
        ...
        if (status) {
            ...
            CmTag.sendMsg(mta.getApplyUserId(), "您提交的【专技岗位过程信息】信息修改申请(序号：{0})未通过审核，请进入干部个人信息修改申请模块查看详情", mta.getId()+"");
        }
