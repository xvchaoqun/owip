...
public interface StatMemberMapper {
    ...
    @Select("select t1.c1 + t2.c2 " +
            ...
            "(select ifnull(count(id),0) as c2 from ow_party where direct_type is not null and find_in_set(#{metaId},direct_type) AND is_deleted = 0) t2")
    ...
    @Select("select t1.c1 + t2.c2 " +
            ...
            "(select ifnull(count(id),0) as c2 from ow_party where direct_type is not null and find_in_set(#{metaId},direct_type) AND is_deleted = 0 and id=#{partyId}) t2")
