...
public interface IDispatchMapper {
    ...
    // 根据发文号查找单位发文
    List<DispatchUnit> selectDispatchUnitByCodeList(@Param("search") String code,
                                                    @Param("unitId") int unitId, RowBounds rowBounds);
    int countDispatchByCodeUnitList(@Param("search") String code,
    ...
    @Select("select dcv.* from dispatch_cadre_view dcv left join cadre_view cv " +
            "on cv.id=dcv.cadre_id where dcv.unit_id=#{unitId} and dcv.type=2 " +
            "and dcv.year between 2013 and 2017 order by  dcv.year desc, cv.sort_order desc")
    List<DispatchCadreView> leaveDispatchCadres(@Param("unitId") int unitId);

    // 更新发文提交的干部任免数量
    /*@Update("update dispatch bd, (select dispatch_id, sum(IF(type=1, 1, 0)) as real_appoint_count,
    sum(IF(type=2, 1, 0)) as real_dismiss_count from dispatch_cadre group by dispatch_id) bdc
    set bd.real_appoint_count= bdc.real_appoint_count, " +
