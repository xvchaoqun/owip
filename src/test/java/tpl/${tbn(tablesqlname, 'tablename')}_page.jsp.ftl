            ...
            <c:if test="&{commonList.recNum>0}">
                <table class="table table-striped table-bordered table-hover table-condensed">
                    ...
                    <tr>
                        ...
                        <shiro:hasPermission name="${tableName}:changeOrder">
                    ...
                    <tbody>
                    <c:forEach items="&{${tableName}s}" var="${tableName}" varStatus="st">
                        <tr>
                            <td class="center">
                                <label class="pos-rel">
                                    <input type="checkbox" value="&{${tableName}.id}" class="ace">
                                    <span class="lbl"></span>
                                </label>
                            </td>
							<#list tableColumns as column>
								<td>&{${tableName}.${tbn(column.name, "tableName")}}</td>
							</#list>
                            <shiro:hasPermission name="${tableName}:changeOrder">
                            <c:if test="&{!_query && commonList.recNum>1}">
                                <td class="hidden-480">
                                    <a href="#" <c:if test="&{commonList.pageNo==1 && st.first}">style="visibility: hidden"</c:if> class="changeOrderBtn" data-id="&{${tableName}.id}" data-direction="1" title="上升"><i class="fa fa-arrow-up"></i></a>
                                    <input type="text" value="1"
                                           class="order-step tooltip-success" data-rel="tooltip" data-placement="top" title="修改操作步长">
                                    <a href="#" <c:if test="&{commonList.pageNo>=commonList.pageNum && st.last}">style="visibility: hidden"</c:if> class="changeOrderBtn" data-id="&{${tableName}.id}" data-direction="-1" title="下降"><i class="fa fa-arrow-down"></i></a>                                </td>
                                </td>
                            </c:if>
                            </shiro:hasPermission>
                            <td>
                                <div class="hidden-sm hidden-xs action-buttons">
                                    <shiro:hasPermission name="${tableName}:edit">
                                    <button data-id="&{${tableName}.${tbn(key, "tableName")}}" class="editBtn btn btn-mini">
                                        <i class="fa fa-edit"></i> 编辑
                                    </button>
                                     </shiro:hasPermission>
                                     <shiro:hasPermission name="${tableName}:del">
                                    <button class="delBtn btn btn-danger btn-mini" data-id="&{${tableName}.${tbn(key, "tableName")}}">
                                ...
                                <div class="hidden-md hidden-lg">
                                    <div class="inline pos-rel">
                                        ...
                                        <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                            ...
                                            <shiro:hasPermission name="${tableName}:edit">
                                            <li>
                                                <a href="#" data-id="&{${tableName}.${tbn(key, "tableName")}}" class="editBtn tooltip-success" data-rel="tooltip" title="编辑">
                                                    <span class="green">
                                            ...
                                            <shiro:hasPermission name="${tableName}:del">
                                            <li>
                                                <a href="#" data-id="&{${tableName}.${tbn(key, "tableName")}}" class="delBtn tooltip-error" data-rel="tooltip" title="删除">
                                                    <span class="red">
